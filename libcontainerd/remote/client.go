...
func (c *client) Create(ctx context.Context, id string, ociSpec *specs.Spec, runtimeOptions interface{}) (containerd.Container, error) {
	container, err := c.client.NewContainer(ctx, id,
		containerd.WithSpec(ociSpec),
		containerd.WithRuntime(runtimeName, runtimeOptions),
		withBundle(filepath.Join(c.stateDir, id), ociSpec),
	)
	if err != nil {
...
func (c *client) Start(ctx context.Context, id, checkpointDir string, withStdin bool, attachStdio libcontainerdtypes.StdioCallback) (int, error) {
	...
	t, err = ctr.NewTask(ctx,
		func(id string) (cio.IO, error) {
			fifos := newFIFOSet(c.bundleDir(id), libcontainerdtypes.InitProcessName, withStdin, spec.Process.Terminal)

			rio, err = c.createIO(ctx, fifos, id, libcontainerdtypes.InitProcessName, stdinCloseSync, attachStdio)
			return rio, err
		},
...
func (c *client) Exec(ctx context.Context, containerID, processID string, spec *specs.Process, withStdin bool, attachStdio libcontainerdtypes.StdioCallback) (int, error) {
	...
	fifos := newFIFOSet(c.bundleDir(containerID), processID, withStdin, spec.Terminal)
	...
	p, err = t.Exec(ctx, processID, spec, func(id string) (cio.IO, error) {
		rio, err = c.createIO(ctx, fifos, containerID, processID, stdinCloseSync, attachStdio)
		return rio, err
	})
	if err != nil {
		close(stdinCloseSync)
...
func (c *client) SignalProcess(ctx context.Context, containerID, processID string, signal int) error {
	process, err := c.getProcess(ctx, containerID, processID)
	if err != nil {
		return wrapError(err)
	}
	return wrapError(process.Kill(ctx, syscall.Signal(signal)))
}
...
func (c *client) Delete(ctx context.Context, containerID string) error {
	...
	if err := ctr.Delete(ctx, c.withBundleCleanup(filepath.Join(c.stateDir, containerID))); err != nil {
		return wrapError(err)
	}
	return nil
}
...
func (c *client) createIO(ctx context.Context, fifos *cio.FIFOSet, containerID, processID string, stdinCloseSync chan struct{}, attachStdio libcontainerdtypes.StdioCallback) (cio.IO, error) {
	var (
		io  *cio.DirectIO
		err error
	)
...
func (c *client) processEvent(ctx context.Context, et libcontainerdtypes.EventType, ei libcontainerdtypes.EventInfo) {
	c.eventQ.Append(ei.ContainerID, func() {
		...
		if et == libcontainerdtypes.EventExit && ei.ProcessID != ei.ContainerID {
			...
			if _, err := c.getContainer(ctx, ei.ContainerID); err != nil {
				c.logger.WithError(err).WithFields(logrus.Fields{
					"container": ei.ContainerID,
				}).Error("failed to find container")
			} else {
				newFIFOSet(c.bundleDir(ei.ContainerID), ei.ProcessID, true, false).Close()
			}
		}
	})
}
...
func (c *client) processEventStream(ctx context.Context, ns string) {
	...
	for {
		...
		case ev = <-eventStream:
			...
			if oomEvent {
				oomKilled[ei.ContainerID] = true
			}
			ei.OOMKilled = oomKilled[ei.ContainerID]
...
func (c *client) getProcess(ctx context.Context, containerID, processID string) (containerd.Process, error) {
	...
	process, err := task.LoadProcess(ctx, processID, nil)
	if err != nil {
		return nil, wrapError(err)
	}
	return process, nil
...
func (c *client) withBundleCleanup(path string) containerd.DeleteOpts {
	return func(ctx context.Context, _ *containerd.Client, ct containers.Container) error {
