...
mount_directory() {
	if [ -z "$_DOCKERD_ROOTLESS_CHILD" ]; then
		echo "mount_directory should be called from the child context. Otherwise data loss is at risk"
		exit 1
	fi

	local directory="$1"
	local mount_options="${2:---bind}"
...
if [ -z "$_DOCKERD_ROOTLESS_CHILD" ]; then
	...
	# specification files.
	[ -d "/etc/cdi" ] && mount_directory /etc/cdi
	[ -d "/var/run/cdi" ] && mount_directory /var/run/cdi
