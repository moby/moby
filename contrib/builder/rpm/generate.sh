...
for version in "${versions[@]}"; do
	...
	case "$from" in
		opensuse:*)
			...
			if [[ "$from" == "opensuse:12."* ]]; then
				# add the repo to pull systemd-rpm-macros from
				echo "RUN zypper ar http://download.opensuse.org/distribution/13.2/repo/oss oss-13" >> "$version/Dockerfile"
				echo "RUN zypper --non-interactive install oss-13:systemd-rpm-macros" >> "$version/Dockerfile"
			fi
			;;
		*)
