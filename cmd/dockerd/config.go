...
func installCommonConfigFlags(conf *config.Config, flags *pflag.FlagSet) {
	...
	flags.Var(&conf.HostGatewayIP, "host-gateway-ip", "IP address that the special 'host-gateway' string in --add-host resolves to. Defaults to the IP addresses of the default bridge")
