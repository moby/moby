...
func (tr *authTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if tr.alwaysSetBasicAuth {
		//debug:
		fmt.Println("Always setting basic auth")
	...
	if req.Header.Get("Authorization") == "" {
		if req.Header.Get("X-Docker-Token") == "true" {
			...
			//debug:
			fmt.Println("Reusing cached token")
	...
	if len(resp.Header["X-Docker-Token"]) > 0 {
		...
		//debug:
		fmt.Println("set token to", tr.token)
	}
	return resp, err
}

// TODO: remove authConfig param once registry client v2 is vendored
...
// TODO: remove this once registry client v2 is vendored
