...
DOCKER_RUN_DOCS := docker run --rm -it -p $(if $(DOCSPORT),$(DOCSPORT):)8000 $(if $(DOCSDIR),-v $(shell pwd)/$(DOCSDIR):/$(DOCSDIR))
...
docs-shell: docs-build
	$(DOCKER_RUN_DOCS) --env AWS_S3_BUCKET "$(DOCKER_DOCS_IMAGE)" bash

docs-release: docs-build
	$(DOCKER_RUN_DOCS) --env AWS_S3_BUCKET "$(DOCKER_DOCS_IMAGE)" ./release.sh
