...
**MARK**
  -- `MARK`
  The **MARK** instruction marks the current position in the build for later
  use with the **SQUASH** command. See **SQUASH** for more details.

**SQUASH**
  -- `SQUASH [command]`
  The **SQUASH** instruction compresses one or more layers into a single, new,
  renamed layer. **SQUASH** must be called after the **MARK** instruction, and
  will rewrite all the layers between the **MARK** and the **SQUASH** into one
  layer.  For example, if there are two commands between the **MARK** and the
  **SQUASH**, it will replace the two layers immediately previous to the squash
  instruction with a new layer consisting of the combined contents of both
  layers, and all the metadata of the most recent -- except with a different
  `ContainerConfig.Cmd`, which is the bit visible in `docker history`. This
  instruction can be used to compress multiple layers for efficiency, and to
  create more semantically-meaningful hierarchies as viewed by `docker
  history`.

  The following example results in a single layer, with a description of "new
  description".
  ...
  ADD a
  RUN b
  SQUASH new description
  ```

  This example also results in a single layer. Note that the command can be
  specified as a JSON array. In this case, we've chosen the command to be the
  same as the one autogenerated by the **RUN** instruction, to make it appear
  as if the three preceeding instructions never happened, even though their
  results will all be manifested in the squashed image.
  ...
  ENV k=v
  RUN b
  SQUASH ["/bin/sh", "-c", "#(nop) RUN b"]
  ```

# HISTORY
*May 2014, Compiled by Zac Dover (zdover at redhat dot com) based on docker.com Dockerfile documentation.
*Feb 2015, updated by Brian Goff (cpuguy83@gmail.com) for readability
*Apr 2015, updated by Burke Libbey (burke@libbey.me) for MARK and SQUASH commands
