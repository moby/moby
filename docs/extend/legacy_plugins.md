...
[HBM plugin](https://github.com/kassisol/hbm)                | HBM (Harbormaster) is an authorization plugin that manages usage of certains commands or parameters passed to Docker daemon. For example, control which images can be pulled or start containers with certains parameters such as: --privileged, --{ipc,net,pid,userns,uts}=host, any Linux capabilities with parameter --cap-add=[], any devices added with parameter --device=[], any dns servers added with parameter --dns=, any ports added with parameter --port=, any volumes mounted with parameter -v, any logging with parameters "--log-driver" and "--log-opt".
