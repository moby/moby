...
Use `docker system events` to get real-time events from the server. These events differ
...
The following commands show several different ways to filter the `docker system events`
output.
