...
The `status` filter matches containers by status. You can filter using `created`, `restarting`, `running`, `paused`, `dead` and `exited`. For example, to filter for `running` containers:
