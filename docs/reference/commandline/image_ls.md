...
+++
title = "image ls"
description = "The image ls command description and usage"
...
The default `docker image ls` will show all top level
...
The following filter matches images with the `com.example.version` label with the `1.0` value.
    ...
    $ docker image ls --filter "label=com.example.version=1.0"
    REPOSITORY          TAG                 IMAGE ID            CREATED              SIZE
