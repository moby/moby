...
avoid the prompts, the `--authn-opt` option can be used to pass in specific
prompted-for values in the form of `key`=`value` ahead of time, or the
`--authn-opt interactive=false` option can be used to treat any attempts to
...
attempt to prompt for the user name.
    ...
    $ docker --authn-opt basic.username=testuser run
...
client as the token.
    ...
    $ docker --authn-opt bearer.token=WUVTLUktQU0tQS1CRUFS run
...
behavior, preventing prompting when it would normally be used.
    ...
    $ docker --authn-opt interactive=false run
