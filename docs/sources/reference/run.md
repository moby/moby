...
`stdout`, `stderr`) you'd like to connect instead, as in:
...
When a developer builds an image from a Dockerfile \<dockerbuilder\> or
when she commits it, the developer can set a number of default
parameters that take effect when the image starts up as a container.
