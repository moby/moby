...
func (b *Builder) readAndParseDockerfile(rc io.ReadCloser) (*parser.Result, error) {
	br := bufio.NewReader(rc)
	if _, err := br.Peek(1); err != nil {
		if err == io.EOF {
			return nil, errors.Errorf("the Dockerfile (%s) cannot be empty", b.options.Dockerfile)
		}
