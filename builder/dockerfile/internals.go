...
func (b *Builder) commit(id string, autoCmd strslice.StrSlice, comment string) error {
	...
	if id == "" {
		...
		if runtime.GOOS != "windows" {
			b.runConfig.Cmd = []string{"/bin/sh", "-c", "#(nop) " + comment}
		} else {
			b.runConfig.Cmd = []string{"cmd", "/S /C", "REM (nop) " + comment}
		}
...
func (b *Builder) runContextCommand(args []string, allowRemote bool, allowLocalDecompression bool, cmdName string) error {
	...
	cmd := b.runConfig.Cmd
	if runtime.GOOS != "windows" {
		b.runConfig.Cmd = []string{"/bin/sh", "-c", fmt.Sprintf("#(nop) %s %s in %s", cmdName, srcHash, dest)}
	} else {
		b.runConfig.Cmd = []string{"cmd", "/S", "/C", fmt.Sprintf("REM (nop) %s %s in %s", cmdName, srcHash, dest)}
	}
	defer func(cmd strslice.StrSlice) { b.runConfig.Cmd = cmd }(cmd)
...
func (b *Builder) run(cID string) (err error) {
	...
	if ret, _ := b.docker.ContainerWait(cID, -1); ret != 0 {
		...
		return &jsonmessage.JSONError{
			Message: fmt.Sprintf("The command '%s' returned a non-zero code: %d", b.runConfig.Cmd.String(), ret),
			Code:    ret,
		}
	}
