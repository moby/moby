...
othergrp:x:6666:
	...
	for _, testcase := range []struct {
		...
		t.Run(testcase.name, func(t *testing.T) {
			idPair, err := parseChownFlag(context.Background(), testcase.builder, testcase.state, testcase.chownStr, contextDir, testcase.idMapping)
	...
	for _, testcase := range []struct {
		...
		t.Run(testcase.name, func(t *testing.T) {
			_, err := parseChownFlag(context.Background(), testcase.builder, testcase.state, testcase.chownStr, contextDir, testcase.idMapping)
