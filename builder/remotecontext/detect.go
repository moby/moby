...
func Detect(config backend.BuildConfig) (remote builder.Source, dockerfile *parser.Result, _ error) {
	...
	switch {
	case remoteURL == "":
		return newArchiveRemote(config.Source, dockerfilePath)
	...
	case urlutil.IsGitURL(remoteURL):
		return newGitRemote(remoteURL, dockerfilePath)
	case urlutil.IsURL(remoteURL):
		return newURLRemote(remoteURL, dockerfilePath, config.ProgressWriter.ProgressReaderFunc)
	default:
