...
func (r *Resolver) setupNAT() error {
	...
	if nftables.Enabled() {
		return r.setupNftablesNAT(laddr, ltcpaddr, resolverIP, ipPort, tcpPort)
	}
...
func (r *Resolver) setupNftablesNAT(laddr, ltcpaddr, resolverIP, ipPort, tcpPort string) error {
	...
	if err := r.backend.ExecFunc(func() {
		setupErr = table.Apply(context.TODO())
	}); err != nil {
		return err
	}
	return setupErr
