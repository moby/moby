...
func (nDB *NetworkDB) Watch(tname, nid string) (*events.Channel, func()) {
	...
	if tname == "" {
		...
		nDB.indexes[byNetwork].Root().WalkPrefix(prefix, func(path []byte, e interface{}) bool {
			v := e.(*entry)
		...
		nDB.indexes[byTable].Root().WalkPrefix(prefix, func(path []byte, e interface{}) bool {
			v := e.(*entry)
			if !v.deleting && v.node != nDB.config.NodeID {
				tuple := strings.SplitN(string(path[1:]), "/", 3)
				if len(tuple) == 3 {
