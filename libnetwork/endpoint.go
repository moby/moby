...
func (ep *Endpoint) sbJoin(ctx context.Context, sb *Sandbox, options ...EndpointOption) (retErr error) {
	...
	if ep == gwepAfter4 || ep == gwepAfter6 {
		...
		if gwepBefore4 != nil && (ep == gwepAfter4 || restartGw4) {
			...
			undoFunc, err := gwepBefore4.revokeExternalConnectivity(ctx)
			if err != nil {
				return err
			}
			if restartGw4 {
				...
				if err := undoFunc(labelsAfter); err != nil {
				...
				defer func() {
					if retErr != nil {
						...
						if err := undoFunc(labelsBefore); err != nil {
							log.G(ctx).WithFields(log.Fields{
								"endpointName": ep.Name(),
								"endpointId":   ep.ID(),
								"role":         role,
		...
		if ep == gwepAfter6 && gwepBefore6 != nil && gwepBefore6 != gwepBefore4 {
			...
			undoFunc, err := gwepBefore6.revokeExternalConnectivity(ctx)
			if err != nil {
				return err
			}
			defer func() {
				if retErr != nil {
					if err := undoFunc(sb.Labels()); err != nil {
						log.G(ctx).WithFields(log.Fields{
							"endpointName": ep.Name(),
							"endpointId":   ep.ID(),
							"error":        err,
...
func (ep *Endpoint) revokeExternalConnectivity(ctx context.Context) (func(map[string]interface{}) error, error) {
	extN, err := ep.getNetworkFromStore()
	if err != nil {
	...
	return func(labels map[string]interface{}) error {
	...
	if moveExtConn4 || moveExtConn6 {
		...
		if gwepAfter4 != nil {
			...
			if restartGw4 {
				...
				if undoFunc, err := gwepBefore4.revokeExternalConnectivity(ctx); err != nil {
					log.G(ctx).WithError(err).Error("Failed to restart IPv4 gateway")
				} else if err := undoFunc(labels); err != nil {
