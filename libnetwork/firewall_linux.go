...
func (c *Controller) selectFirewallBackend() error {
	...
	if c.cfg.FirewallBackend == "nftables" {
		...
		if iptables.UsingFirewalld() {
			return fmt.Errorf("firewall-backend is set to nftables, but firewalld is running")
		}
	...
	if os.Getenv("DOCKER_FIREWALL_BACKEND") == "nftables" {
		if iptables.UsingFirewalld() {
			return fmt.Errorf("firewall-backend is not set and DOCKER_FIREWALL_BACKEND=nftables, but firewalld is running")
		}
		if err := nftables.Enable(); err != nil {
