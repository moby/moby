...
func (d *driver) peerDbNetworkWalk(nid string, f func(netip.Prefix, peerEntry) bool) error {
	d.peerDb.Lock()
...
func (d *driver) initSandboxPeerDB(nid string) {
	...
	err := d.peerDbNetworkWalk(nid, func(peerIP netip.Prefix, pEntry peerEntry) bool {
		if !pEntry.isLocal() {
