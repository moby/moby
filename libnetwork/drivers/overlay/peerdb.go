...
func (pm *peerMap) Walk(f func(netip.Prefix, peerEntry) bool) {
	for _, peerAddr := range pm.mp.Keys() {
		entry, ok := pm.Get(peerAddr)
		if ok && f(peerAddr, entry) {
			return
		}
	}
}
...
func (n *network) initSandboxPeerDB() error {
	var errs []error
	n.peerdb.Walk(func(peerIP netip.Prefix, pEntry peerEntry) bool {
		if !pEntry.isLocal() {
...
func (n *network) deleteNeighbor(peerIP netip.Prefix, peerMac net.HardwareAddr, vtep netip.Addr) error {
	...
	if n.fdbCnt.Add(ipmacOf(vtep, peerMac), -1) == 0 {
		if err := n.sbox.DeleteNeighbor(vtep.AsSlice(), peerMac, osl.WithLinkName(s.vxlanName)); err != nil {
			return fmt.Errorf("could not delete fdb entry in the sandbox: %w", err)
		}
	}

	// Delete neighbor entry for the peer IP
	if err := n.sbox.DeleteNeighbor(peerIP.Addr().AsSlice(), peerMac, osl.WithLinkName(s.vxlanName), osl.WithFamily(syscall.AF_BRIDGE)); err != nil {
