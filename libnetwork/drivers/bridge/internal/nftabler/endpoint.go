...
func (n *network) filterDirectAccess(ctx context.Context, table nftables.TableRef, conf firewaller.NetworkConfigFam, epIP netip.Addr, enable bool) error {
	...
	ifNames := strings.Join(append(n.config.TrustedHostInterfaces, n.config.IfName), ", ")
	return updater(rawPreroutingPortsRuleGroup,
		`%s daddr %s iifname != { %s } counter drop comment "DROP DIRECT ACCESS"`,
		table.Family(), epIP, ifNames)
}
