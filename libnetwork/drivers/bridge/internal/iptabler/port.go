...
func (n *Network) setPerPortIptables(ctx context.Context, b types.PortBinding, enable bool) error {
	...
	if err := n.dropLegacyFilterDirectAccess(ctx, b, enable); err != nil {
		return err
	}
...
func (n *Network) dropLegacyFilterDirectAccess(ctx context.Context, b types.PortBinding, enable bool) error {
	if rawRulesDisabled(ctx) {
		return nil
	}
	ipv := iptables.IPv4
