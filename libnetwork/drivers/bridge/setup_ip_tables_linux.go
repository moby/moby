...
func (n *iptablesNetwork) configure(ipv iptables.IPVersion, conf networkConfigFam) error {
	...
	if !n.Internal {
		...
		if err := setINC(ipv, n.IfName, conf.Routed, true); err != nil {
			return err
		}
	}
	return nil
...
func (n *iptablesNetwork) cleanup() error {
	var errs []error
...
func setDefaultForwardRule(ipVersion iptables.IPVersion, ifName string, config networkConfigFam) (iptableCleanFunc, error) {
...
func (n *iptablesNetwork) setupNonInternalNetworkRules(ipVer iptables.IPVersion, config networkConfigFam, enable bool) error {
	...
	if config.HostIP.IsValid() {
		...
		hostAddr := config.HostIP.Unmap().String()
