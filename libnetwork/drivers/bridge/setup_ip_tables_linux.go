...
func setupIPChains(config configuration, version iptables.IPVersion) (natChain *iptables.ChainInfo, filterChain *iptables.ChainInfo, isolationChain1 *iptables.ChainInfo, isolationChain2 *iptables.ChainInfo, retErr error) {
	...
	if err := mirroredWSL2Workaround(version); err != nil {
		return nil, nil, nil, nil, err
	}
...
func mirroredWSL2Workaround(ipv iptables.IPVersion) error {
	...
	return programChainRule(mirroredWSL2Rule(), "WSL2 loopback", mirroredWSL2())
}

// mirroredWSL2 returns true if the host Linux appears to be running under
// Windows WSL2 with mirrored mode networking. If a loopback0 device exists, and
// there's an executable at /usr/bin/wslinfo, infer that this is WSL2 with
// mirrored networking. ("wslinfo --networking-mode" reports "mirrored", but
// applying the workaround for WSL2's loopback device when it's not needed is
// low risk, compared with executing wslinfo with dockerd's elevated permissions.)
