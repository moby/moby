...
func CheckTasks(ctx context.Context, s *store.MemoryStore, readTx store.ReadTx, initHandler InitHandler, startSupervisor restart.SupervisorInterface) error {
	...
	err := s.Batch(func(batch *store.Batch) error {
		...
		for _, t := range tasks {
			...
			if t.DesiredState != api.TaskState_READY || t.Status.GetState() > api.TaskState_COMPLETE {
				continue
			}
			restartDelay := defaults.Service.Task.Restart.Delay.AsDuration()
			if d := t.Spec.GetRestart().GetDelay(); d != nil {
				// AsDuration cannot fail, so validate explicitly to keep
				// falling back to the default for a malformed delay.
				if d.IsValid() {
					restartDelay = d.AsDuration()
				} else {
					log.G(ctx).Error("invalid restart delay")
				}
			}
			if restartDelay != 0 {
				src := t.Status.GetAppliedAt()
				if src == nil {
					src = t.Status.GetTimestamp()
				}
				if src.IsValid() {
					timestamp := src.AsTime()
					...
					if restartDelay > 0 {
						_ = batch.Update(func(tx store.Tx) error {
							...
							if t == nil || t.DesiredState != api.TaskState_READY {
								return nil
							}
					...
					log.G(ctx).Error("invalid status timestamp")
				}
			}
