...
func newRootRotationObject(ctx context.Context, securityConfig *ca.SecurityConfig, apiRootCA *api.RootCA, newCARootCA ca.RootCA, extCAs []*api.ExternalCA, version uint64) (*api.RootCA, error) {
	...
	default:
		...
		for _, c := range extCAs {
			if c.Protocol == api.ExternalCA_CFSSL {
...
func validateHasAtLeastOneExternalCA(ctx context.Context, externalCAs map[string][]*api.ExternalCA, securityConfig *ca.SecurityConfig,
	...
	if ok {
		...
		for i, ca := range specific {
			if ca.Protocol == api.ExternalCA_CFSSL {
...
func validateCAConfig(ctx context.Context, securityConfig *ca.SecurityConfig, cluster *api.Cluster) (*api.RootCA, error) {
	newConfig := cluster.Spec.GetCaConfig().Copy()
	if newConfig == nil {
		// CaConfig used to be a non-nullable embedded message, so callers
		// could always reach through it.
	...
	normalizedRootCA := ca.NormalizePEMs(cluster.RootCa.GetCaCert())
	...
	if len(newConfig.SigningCaCert) == 0 {
		if cluster.RootCa.GetLastForcedRotation() != newConfig.ForceRotate {
		...
		// still valid
		if cluster.RootCa.GetRootRotation() != nil && !hasSigningKey(cluster.RootCa.GetRootRotation()) {
			_, err := validateHasAtLeastOneExternalCA(ctx, extCAs, securityConfig, ca.NormalizePEMs(cluster.RootCa.GetRootRotation().GetCaCert()), "next")
			if err != nil {
				return nil, err
			}
		}
	...
	// key.
	if len(newConfig.SigningCaKey) > 0 && bytes.Equal(newConfig.SigningCaKey, cluster.RootCa.GetCaKey()) {
	...
	if r := cluster.RootCa.GetRootRotation(); r != nil && bytes.Equal(ca.NormalizePEMs(r.CaCert), newConfig.SigningCaCert) {
		copied := cluster.RootCa.Copy()
