...
//
// If the stream ends or the context is canceled, the underlying [io.ReadCloser]
...
func (cli *Client) ContainerStats(ctx context.Context, containerID string, options ContainerStatsOptions) (ContainerStatsResult, error) {
	...
	if options.Stream {
		...
		// Note: daemons may return an error if both set: "cannot have stream=true and one-shot=true"
		//
