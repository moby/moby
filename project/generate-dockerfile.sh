...
# To enable gccgo based builds, set environment variable ARCH
# for example export ARCH=amd64 or ppc64le or s390x would enable gccgo build on that architecture.
# By default ARCH variable is not set and the Dockerfile generated will use GC to build source code. 
#
...
if [ -z "$ARCH" ]; then 
	...
	cat <<-'EOF'
		...
		WORKDIR /work
		RUN svn co -q --revision 219661 svn://gcc.gnu.org/svn/gcc/trunk src
