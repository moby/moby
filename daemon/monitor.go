...
func (daemon *Daemon) handleContainerExit(ctx context.Context, c *container.Container, e *libcontainerdtypes.EventInfo) error {
	...
	if restart {
		go func() {
			ctx, span := otel.Tracer("").Start(context.Background(), "Daemon.handleContainerExit.restart",
				trace.WithAttributes(attribute.String("container.id", c.ID)),
			...
			if waitErr == nil {
				...
				if err := daemon.containerStart(ctx, daemon.config(), c, "", "", false); err != nil {
...
func (daemon *Daemon) ProcessEvent(id string, e libcontainerdtypes.EventType, ei libcontainerdtypes.EventInfo) (retErr error) {
	ctx, span := otel.Tracer("").Start(context.Background(), "Daemon.ProcessEvent", trace.WithAttributes(
	...
	case libcontainerdtypes.EventExit:
		...
		if execConfig := c.ExecCommands.Get(ei.ProcessID); execConfig != nil {
			...
			if execConfig.Process != nil {
				go func() {
					if _, err := execConfig.Process.Delete(ctx); err != nil {
						log.G(ctx).WithFields(log.Fields{
							"error":     err,
	...
	case libcontainerdtypes.EventStart:
		...
		if !c.State.Running {
			ctr, err := daemon.containerd.LoadContainer(ctx, c.ID)
			if err != nil {
