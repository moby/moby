...
func (daemon *Daemon) ProcessEvent(id string, e libcontainerdtypes.EventType, ei libcontainerdtypes.EventInfo) error {
	c, err := daemon.getContainerByC8dContainerID(id)
	if err != nil {
	...
	case libcontainerdtypes.EventOOM:
		...
		c.Lock()
		defer c.Unlock()
		if !isCurrentRunEventWithLock(c, ei.ContainerID) {
			return nil
		}
	...
	case libcontainerdtypes.EventExit:
		...
		c.Lock()
		currentEvent := isCurrentRunEventWithLock(c, ei.ContainerID)
		c.Unlock()
		if !currentEvent {
			return nil
		}
	...
	case libcontainerdtypes.EventStart:
		c.Lock()
		defer c.Unlock()
		// Ignore previous runs, including externally started tasks that use
		// the legacy Docker ID.
		if !isCurrentRunEventWithLock(c, ei.ContainerID) {
			return nil
		}
		...
		if !c.State.Running {
			ctr, err := daemon.containerd.LoadContainer(context.Background(), c.C8dContainerID())
			if err != nil {
...
func (daemon *Daemon) shouldIgnoreExitEventWithLock(c *container.Container, e *libcontainerdtypes.EventInfo) (ret bool) {
	...
	defer func() {
		if ret {
			log.G(context.TODO()).WithFields(log.Fields{
				"container":        c.ID,
				"c8dContainerID":   c.C8dContainerID(),
				"eventContainerID": e.ContainerID,
				"state":            c.State.State(),
				"exitCode":         e.ExitCode,
				"exitedAt":         e.ExitedAt,
	...
	if !isCurrentRunEventWithLock(c, e.ContainerID) {
		return true
	}
	...
	case containertypes.StateRunning:
		if c.State.RunID != 0 {
			// The per-run ID already proves that this event is current.
			return false
		}

		// Legacy runs reused the Docker ID, so use task status to reject an
		// exit event from an earlier run.
...
func isCurrentRunEventWithLock(c *container.Container, containerID string) bool {
	return containerID == c.C8dContainerID()
}
