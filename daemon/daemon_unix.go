...
func (o defBrOptsV6) defGw() (gw net.IP, optName string) {
	return o.cfg.DefaultGatewayIPv6, "DefaultGatewayIPv6"
}
...
type defBrOpts interface {
	...
	defGw() (gw net.IP, optName string)
}
...
func getDefaultBridgeIPAMConf(
	...
	if cfgBIP, _ := opts.bip(); cfgBIP != "" {
		if bIP, bIPNet, err = net.ParseCIDR(cfgBIP); err != nil {
			return nil, err
		}
	} else {
		if bIP, bIPNet, err = selectBIP(bridgeName, opts.nlFamily(), fCidrIP, fCidrIPNet); err != nil {
			return nil, err
		}
	...
	if fCidrIP != nil && fCidrIPNet != nil {
		...
		if ipamConf.PreferredPool == "" {
			...
			if !bIPNet.Contains(fCidrIP) || (fCidrOnes < bIPOnes) {
				...
				}).Warn(fixedCIDROpt + " is outside any subnet implied by addresses on the user-managed default bridge")
	...
	if defGw, defGwOpt := opts.defGw(); defGw != nil {
		ipamConf.AuxAddresses[defGwOpt] = defGw.String()
	}
