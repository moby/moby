...
func getDefaultBridgeIPAMConf(
	...
	if cfg.IP != "" {
		...
		if bIP, bIPNet, err = selectBIP(bridgeName, family, fCidrIP, fCidrIPNet); err != nil {
			return nil, err
		}
	...
	if fCidrIP != nil && fCidrIPNet != nil {
		...
		if ipamConf.PreferredPool == "" {
			...
			if !bIPNet.Contains(fCidrIP) || (fCidrOnes < bIPOnes) {
				...
				}).Warn("fixed-cidr is outside the subnet implied by addresses on the user-managed default bridge")
				ipamConf.SubPool = ""
			}
		}
	}
