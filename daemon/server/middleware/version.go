...
func NewVersionMiddleware(serverVersion, defaultAPIVersion, minAPIVersion string) (*VersionMiddleware, error) {
	if versions.LessThan(defaultAPIVersion, versions.Min) || versions.GreaterThan(defaultAPIVersion, config.DefaultAPIVersion) {
		return nil, fmt.Errorf("invalid default API version (%s): must be between %s and %s", defaultAPIVersion, versions.Min, config.DefaultAPIVersion)
	}
	if versions.LessThan(minAPIVersion, versions.Min) || versions.GreaterThan(minAPIVersion, config.DefaultAPIVersion) {
		return nil, fmt.Errorf("invalid minimum API version (%s): must be between %s and %s", minAPIVersion, versions.Min, config.DefaultAPIVersion)
	}
