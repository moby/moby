...
// WriteJSONLogStream writes an JSON stream of log messages from the messages channel.
func WriteJSONLogStream(_ context.Context, w http.ResponseWriter, msgs <-chan *backend.LogMessage, config *backend.ContainerLogsOptions) {
...
func newJSONLogWriter(w io.Writer, contentType string, opts *backend.ContainerLogsOptions) *jsonLogWriter {
	encode := NewJSONStreamEncoder(w, contentType)
