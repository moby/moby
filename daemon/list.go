...
// Events returns events generated by daemon
func (daemon *Daemon) Events(ctx context.Context, since, until time.Time, enc *json.Encoder, filter filters.Args) error {
	now := time.Now()
	...
	buffered, l := daemon.SubscribeToEvents(since, until, filter)
	defer daemon.UnsubscribeFromEvents(l)
