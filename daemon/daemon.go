...
func NewDaemon(ctx context.Context, config *config.Config, pluginStore *plugin.Store, authzMiddleware *authorization.Middleware) (daemon *Daemon, err error) {
	...
	if config.ContainerdAddr != "" {
		d.containerdCli, err = remote.NewContainerdClient(config.ContainerdAddr, containerd.WithDefaultNamespace(config.ContainerdNamespace))
		if err != nil {
	...
	createPluginExec := func(m *plugin.Manager) (plugin.Executor, error) {
		...
		if config.ContainerdAddr != "" {
			pluginCli, err = remote.NewContainerdClient(config.ContainerdAddr, containerd.WithDefaultNamespace(config.ContainerdPluginNamespace))
			if err != nil {
				return nil, errors.Wrapf(err, "failed to dial %q", config.ContainerdAddr)
			}
		}
