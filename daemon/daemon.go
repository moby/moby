...
func NewDaemon(ctx context.Context, config *config.Config, pluginStore *plugin.Store, authzMiddleware *authorization.Middleware, containerdDialer ContainerdDialer) (_ *Daemon, retErr error) {
	...
	if d.jobsExtension != nil {
		...
		// Unnamed jobs draw their names from the same generator point as
		// containers.
		generateName := func(ctx context.Context, retry int64) (string, error) {
			reply, err := containernamegeneratorv0.GenerateContainerName(ctx, d.extensionHost, &containernamegeneratorv0.GenerateContainerNameRequest{Retry: retry})
			if reply != nil && reply.Name != "" {
				return reply.Name, nil
			}
			return "", err
		}
		if err := d.jobsExtension.Activate(ctx, jobsBackend{d}, generateName); err != nil {
			return nil, err
		}
	}
