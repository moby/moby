...
func (daemon *Daemon) ContainerStatPath(name string, path string) (*types.ContainerPathStat, error) {
...
func (daemon *Daemon) ContainerArchivePath(name string, path string) (content io.ReadCloser, stat *types.ContainerPathStat, _ error) {
	ctr, err := daemon.GetContainer(name)
	if err != nil {
		return nil, nil, err
	}
