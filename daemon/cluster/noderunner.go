...
func (n *nodeRunner) Stop() error {
	...
	if n.swarmNode == nil {
		...
		// Release n.mu first: SendClusterEvent takes Cluster.mu, and Cluster
		// reads node state by taking Cluster.mu and then n.mu, so sending
		// while holding n.mu inverts that order.
		cluster := n.cluster
		n.mu.Unlock()
		if cluster != nil {
			cluster.SendClusterEvent(lncluster.EventNodeLeave)
		}
		return nil
	}
