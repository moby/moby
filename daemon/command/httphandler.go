...
func supportGRPC(ctx context.Context, lss []net.Listener, hasTLS bool) (bool, http.Protocols) {
	...
	p.SetHTTP1(true)
	var hasUnecryptedTCP bool
	var hasTCP bool
	for _, ls := range lss {
		if ls.Addr().Network() != "unix" {
			hasTCP = true
			if !hasTLS {
				log.G(ctx).WithField("addr", ls.Addr().String()).Warn("Listener is not using TLS, GRPC not enabled")
				hasUnecryptedTCP = true
				break
			}
		}
	}
	if hasUnecryptedTCP {
		// GRPC does not support HTTP/2 over TCP without TLS
		return false, p
	}
	p.SetHTTP2(true)
	if !hasTCP {
