...
func installConfigFlags(conf *config.Config, flags *pflag.FlagSet) {
	...
	flags.StringVar(&conf.NetworkingConfig.BridgeConfig.FixedCIDR, "fixed-cidr", "", "IPv4 subnet for fixed IPs")
	flags.StringVarP(&conf.NetworkingConfig.BridgeConfig.Iface, "bridge", "b", "", "Attach containers to a virtual switch")
