...
func installConfigFlags(conf *config.Config, flags *pflag.FlagSet) {
	...
	flags.BoolVar(&conf.CgroupParentFromClient, "cgroup-parent-from-client", false, "If set, containers inherit the cgroup of the client process via SO_PEERCRED (Unix sockets only, Linux only, mutually exclusive with --cgroup-parent)")
