...
func installCommonConfigFlags(conf *config.Config, flags *pflag.FlagSet) {
	...
	flags.Var(opts.NewNamedMapMapOpts("default-network-opts", conf.Networking.DefaultNetworkOpts, nil), "default-network-opt", "Default network options")
	flags.IntVar(&conf.Networking.MTU, "mtu", conf.Networking.MTU, `Set the MTU for the default "bridge" network`)
	...
	flags.IntVar(&conf.Networking.NetworkControlPlaneMTU, "network-control-plane-mtu", conf.Networking.NetworkControlPlaneMTU, "Network Control plane MTU")
