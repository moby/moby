...
func installCommonConfigFlags(conf *config.Config, flags *pflag.FlagSet) {
	...
	flags.Var(opts.NewNamedListOptsRef("extension-dirs", &conf.ExtensionDirs, nil), "extension-dir", "Directories to load extension binaries from")
	flags.Var(opts.NewNamedListOptsRef("enable-extensions", &conf.EnableExtensions, nil), "enable-extension", "Built-in extensions to enable, by extension ID")
