	...
	if err := daemon.kill(ctx, container, sig); err != nil {
		if errdefs.IsNotFound(err) {
			...
			go func() {
				...
				ctx, cancel := context.WithTimeout(ctx, time.Duration(container.StopTimeout())*time.Second)
				defer cancel()
				s := <-container.Wait(ctx, containerpkg.WaitConditionNotRunning)
				if s.Err() != nil {
					// Use a fresh context.
					// This parent context was passed in from a separate goroutine and may be cancelled... meanwhile we don't want to cancel handling of a container exit.
					daemon.handleContainerExit(context.Background(), container, nil)
				}
			}()
		} else {
