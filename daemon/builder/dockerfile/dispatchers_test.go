...
func TestExpose(t *testing.T) {
	...
	assert.Check(t, is.Contains(sb.state.runConfig.ExposedPorts, container.PortFrom(80, container.TCP)))
}
...
func TestParsePortSpecs(t *testing.T) {
	...
	if _, ok := portMap[container.PortFrom(1234, container.TCP)]; !ok {
		t.Fatal("1234/tcp was not parsed properly")
	}

	if _, ok := portMap[container.PortFrom(2345, container.UDP)]; !ok {
		t.Fatal("2345/udp was not parsed properly")
	}

	if _, ok := portMap[container.PortFrom(3456, container.SCTP)]; !ok {
...
func TestParsePortSpecFull(t *testing.T) {
	...
	expected := []container.PortMap{
		{
			container.PortFrom(3333, container.TCP): []container.PortBinding{
				{
					HostIP:   "0.0.0.0",
					HostPort: "1234",
				},
		...
		{
			container.PortFrom(3334, container.TCP): []container.PortBinding{
				{
					HostIP:   "0.0.0.0",
					HostPort: "1235",
				},
	...
	cases := []test{
		{
			...
			expected: []container.PortMap{
				{
					container.PortFrom(333, container.TCP): []container.PortBinding{
						{
		...
		},
		{
			...
			expected: []container.PortMap{
				{
					container.PortFrom(80, container.TCP): []container.PortBinding{
						{
							HostIP:   "::1",
							HostPort: "80",
						},
...
func TestParsePortSpecsWithRange(t *testing.T) {
	...
	if _, ok := portMap[container.PortFrom(1235, container.TCP)]; !ok {
		t.Fatal("1234-1236/tcp was not parsed properly")
	}

	if _, ok := portMap[container.PortFrom(2346, container.UDP)]; !ok {
