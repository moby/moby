...
// the same protocol restrictions as the rest of the clone, so a repository
// cannot pull a submodule from the daemon host's filesystem when the host's git
// configuration enables the "file" protocol.
//
...
// fails; routing the update through gitWithinDir forces protocol.file.allow=never,
// which blocks it.
func TestCloneSubmoduleFileProtocol(t *testing.T) {
	...
	git := func(dir string, args ...string) {
		t.Helper()
		out, err := gitRepo{}.gitWithinDir(dir, args...)
	...
	checkoutDir, err := gitRepo{remote: server.URL + "/super", ref: "master"}.clone()
	if checkoutDir != "" {
