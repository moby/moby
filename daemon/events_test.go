...
func TestEventTimestamp(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "Create action uses CreatedAt timestamp",
		...
		{
			name:   "Update action uses UpdatedAt timestamp",
		...
		{
			name:   "Remove action uses current time",
			meta:   swarmapi.Meta{},
		...
		{
			name:   "Unknown action returns valid timestamp",
			meta:   swarmapi.Meta{},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := eventTimestamp(tt.meta, tt.action)
			tt.check(t, result)
		})
	}
}
