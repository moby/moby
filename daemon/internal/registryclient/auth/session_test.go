...
func (tcs *testCredentialStore) SetRefreshToken(_ *url.URL, service string, token string) {
...
func TestEndpointAuthorizeToken(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodGet, e+"/v2/hello", http.NoBody)
	...
	req, _ = http.NewRequest(http.MethodGet, e2+"/v2/hello", http.NoBody)
...
func TestEndpointAuthorizeRefreshToken(t *testing.T) {
	...
	tokenMap := testutil.RequestResponseMap([]testutil.RequestResponseMapping{
		{
			...
			Response: testutil.Response{
				StatusCode: http.StatusOK,
				Body:       []byte(fmt.Sprintf(`{"access_token":"statictoken","refresh_token":%q}`, refreshToken1)),
			},
		},
		{
			...
			Response: testutil.Response{
				StatusCode: http.StatusOK,
				Body:       []byte(fmt.Sprintf(`{"access_token":"statictoken","refresh_token":%q}`, refreshToken2)),
			},
		},
		{
			Request: testutil.Request{
	...
	req, _ = http.NewRequest(http.MethodGet, e3+"/v2/hello", http.NoBody)
...
func TestEndpointAuthorizeV2RefreshToken(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodGet, e+"/v1/search", http.NoBody)
	resp, err := client.Do(req)
	if err != nil {
		t.Fatalf("Error sending get request: %s", err)
	}
