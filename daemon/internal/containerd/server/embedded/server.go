...
// persistent state (bolt DB, content store) and stateDir for runtime state
// (shim sockets, the gRPC/ttrpc sockets). See the package doc for the transport
// layout. Plugins self-register via the blank imports above and in the
...
func Start(ctx context.Context, rootDir, stateDir string, opts ...DaemonOpt) (Daemon, error) {
	...
	e := &embeddedDaemon{srv: srv, address: address, pipe: pipe, ttrpcL: ttrpcL, stopCh: make(chan struct{})}
	...
	serve("gRPC", socket, srv.ServeGRPC)
	serve("gRPC", pipe, srv.ServeGRPC)
...
// pipeListener is a net.Listener whose connections are supplied in-memory by
...
func (l *pipeListener) Addr() net.Addr {
	return &net.UnixAddr{Name: "embedded-containerd", Net: "unix"}
}
