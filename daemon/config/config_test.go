	...
	testCases := []struct {
		...
		*/
		...
		},
		{
			...
			platform:    "linux",
			expectedErr: "invalid exec-opt (isolation=default): 'isolation' option is only supported on windows",
		},
		{
			...
			platform:    "windows",
			expectedErr: "invalid exec-opt (native.cgroupdriver=systemd): 'native.cgroupdriver' option is only supported on linux",
		},
	}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
