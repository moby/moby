...
func TestValidateUlimitsSoftHard(t *testing.T) {
	configFileData := `
		{
			"debug": true,
			"default-ulimits": {
	...
	_, err := getConflictFreeConfiguration(file.Path(), flags)
	assert.Error(t, err, "ulimit soft limit must be less than or equal to hard limit: 4096 > 2048")
}

func TestValidateUlimitsName(t *testing.T) {
	configFileData := `
		{
			"debug": true,
			"default-ulimits": {
	...
	_, err := getConflictFreeConfiguration(file.Path(), flags)
	assert.Error(t, err, "invalid ulimit name noffile")
}
