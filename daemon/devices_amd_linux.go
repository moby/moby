...
func createAMDCDIUpdater(l vendorLister) func(*specs.Spec, *deviceInstance) error {
	return func(s *specs.Spec, dev *deviceInstance) error {
		vendor, err := discoverGPUVendor(l)
		if err != nil {
...
func getAMDDeviceDrivers(cdiCache *cdi.Cache) map[string]*deviceDriver {
	...
	amdDrivers := make(map[string]*deviceDriver)
	amdDrivers["amd"] = &deviceDriver{
		capset:     capset,
