...
func (i *ImageService) CreateLayer(ctr *container.Container, initFunc layer.MountInit) (container.RWLayer, error) {
	...
	if !i.idMapping.Empty() {
		return layer, i.remapSnapshot(ctx, sn, id, id+"-init")
	}
...
func (i *ImageService) GetLayerByID(cid string) (container.RWLayer, error) {
	...
	case 1:
		lease = lss[0]
	case 0:
		log.G(ctx).WithField("container", cid).Warn("rw layer lease not found for container")
	default:
		panic("this should never happen, more than one lease has the same id " + cid)
	}
