...
// the image has any hard delete conflicts (child gdstore or running containers
...
// succeeds and prune is true, the parent gdstore are also deleted if they do
// not have any soft or hard delete conflicts themselves. Any deleted gdstore
...
func (i *ImageService) checkImageDeleteConflict(imgID image.ID, mask conflictType) *imageDeleteConflict {
	// Check if the image has any descendant gdstore.
	if mask&conflictDependentChild != 0 && len(i.imageStore.Children(imgID)) > 0 {
		return &imageDeleteConflict{
			hard:    true,
			imgID:   imgID,
			message: "image has dependent child gdstore",
		}
	}
