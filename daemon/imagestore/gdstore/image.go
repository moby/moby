...
func (i *ImageService) GetImage(ctx context.Context, refOrID string, options backend.GetImageOpts) (retImg *image.Image, retErr error) {
	defer func() {
		...
		// Where this gets hairy is the image store does not currently support multi-arch gdstore, e.g.:
		//   An image `foo` may have a multi-arch manifest, but the image store only fetches the image for a specific platform
		//   The image store does not store the manifest list and image tags are assigned to architecture specific gdstore.
		//   So we can have a `foo` image that is amd64 but the user requested armv7. If the user looks at the list of gdstore.
