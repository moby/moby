...
// byCreated is a temporary type used to sort a list of gdstore by creation
// time.
...
// Images returns a filtered list of gdstore.
func (i *ImageService) Images(ctx context.Context, opts imagetypes.ListOptions) ([]*imagetypes.Summary, error) {
	...
	for id, img := range selectedImages {
		...
		// Skip any gdstore with an unsupported operating system to avoid a potential
		// panic when indexing through the layerstore. Don't error as we want to list
		// the other gdstore. This should never happen, but here as a safety precaution.
		...
		if summary.RepoDigests == nil && summary.RepoTags == nil {
			if opts.All || len(i.imageStore.Children(id)) == 0 {
				...
				if opts.Filters.Contains("reference") { // skip gdstore with no references if filtering by reference
					continue
				}
			} else {
				continue
	...
	if opts.SharedSize {
		...
		if danglingOnly {
			// If danglingOnly is true, then selectedImages include only dangling gdstore,
			// but we need to consider all existing gdstore to correctly perform reference counting.
		...
		// Count layer references across all known gdstore
