...
//
// TODO(thaJeztah): verify behavior of `RepoDigests` and `RepoTags` for gdstore without (untagged) or multiple tags; see https://github.com/moby/moby/issues/43861
// TODO(thaJeztah): verify "Size" vs "VirtualSize" in gdstore; see https://github.com/moby/moby/issues/43862
...
func (i *ImageService) singlePlatformImage(ctx context.Context, contentStore content.Store, repoTags []string, imageManifest *ImageManifest) (*imagetypes.Summary, error) {
	...
	if err != nil {
		...
		// TODO: Reconsider when c8dstore is more strict on image names, see:
...
// TODO(thaJeztah): reimplement filters using c8dstore filters if possible: see https://github.com/moby/moby/issues/43845
...
func computeSharedSize(chainIDs []digest.Digest, layers map[digest.Digest]int, sizeFn func(d digest.Digest) (int64, error)) (int64, error) {
	...
	for _, chainID := range chainIDs {
		...
		if err != nil {
			// Several gdstore might share the same layer and neither of them
