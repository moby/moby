...
const (
	...
	contentLabelGcRefContainerConfig = "c8dstore.io/gc.ref.content.moby/container.config"
...
func (rw *rwlayer) Commit() (_ builder.ROLayer, outErr error) {
	...
	// Unmount the layer, required by the c8dstore windows snapshotter.
	...
	diffIDStr, ok := info.Labels["c8dstore.io/uncompressed"]
	if !ok {
...
// writeContentsForImage will commit oci image config and manifest into c8dstore's content store.
func writeContentsForImage(ctx context.Context, snName string, cs content.Store,
	...
	labels := map[string]string{
		"c8dstore.io/gc.ref.content.0": configDesc.Digest.String(),
	}
	for i, l := range layers {
		labels[fmt.Sprintf("c8dstore.io/gc.ref.content.%d", i+1)] = l.Digest.String()
	}
	...
	labelOpt := content.WithLabels(map[string]string{
		fmt.Sprintf("c8dstore.io/gc.ref.snapshot.%s", snName): identity.ChainID(newConfig.RootFS.DiffIDs).String(),
