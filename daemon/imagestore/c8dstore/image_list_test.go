	...
	for _, idx := range index {
		for _, desc := range idx.Manifests {
			imgs = append(imgs, images.Image{
				Name:   desc.Annotations["io.c8dstore.image.name"],
				Target: desc,
			})
		}
	}
...
func BenchmarkImageList(b *testing.B) {
	...
	for _, count := range []int{10, 100, 1000} {
		...
		b.Run(strconv.Itoa(count)+"-gdstore", func(b *testing.B) {
			for i := 0; i < b.N; i++ {
...
func TestImageList(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name:   "two gdstore are two entries",
		...
		{
			name:   "three gdstore, one is an empty index",
