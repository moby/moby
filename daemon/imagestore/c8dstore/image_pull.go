...
import (
	...
	dimages "github.com/docker/docker/daemon/imagestore/gdstore"
...
func (i *ImageService) pullTag(ctx context.Context, ref reference.Named, platform *ocispec.Platform, metaHeaders map[string][]string, authConfig *registrytypes.AuthConfig, out progress.Output) error {
	...
	// Allow pulling application/vnd.docker.distribution.manifest.v1+prettyjws gdstore
	// by converting them to OCI manifests.
	opts = append(opts, containerd.WithSchema1Conversion) //nolint:staticcheck // Ignore SA1019: c8dstore.WithSchema1Conversion is deprecated: use Schema 2 or OCI gdstore.

	img, err := i.client.Pull(ctx, ref.String(), opts...)
	if err != nil {
		if errors.Is(err, docker.ErrInvalidAuthorization) {
			// Match error returned by c8dstore.
		...
		if cerrdefs.IsNotFound(err) {
			// Transform "no match for platform in manifest" error returned by c8dstore into
			// the same message as the graphdrivers backend.
			// The one returned by c8dstore doesn't contain the platform and is much less informative.
