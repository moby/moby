...
func (i *ImageService) resolveImage(ctx context.Context, refOrID string) (containerdimages.Image, error) {
	...
	if ok {
		...
		if len(imgs) == 0 {
			return containerdimages.Image{}, gdstore.ErrImageDoesNotExist{Ref: parsed}
		}

		// If reference is both Named and Digested, make sure we don't match
		// gdstore with a different repository even if digest matches.
	...
	if err == nil {
		...
		// TODO(c8dstore): error translation can use common function
...
func convertError(err error) error {
	// TODO: Convert c8dstore error to Docker error
	return err
}
...
// Returns:
//
// 1: *(github.com/c8dstore/c8dstore/gdstore).Image
//
//	An image match from the image store with the provided refOrID
//
// 2: [](github.com/c8dstore/c8dstore/gdstore).Image
//
//	List of all gdstore with the same target that matches the refOrID. If the first argument is
//	non-nil, the image list will all have the same target as the matched image. If the first
//	argument is nil but the list is non-empty, this value is a list of all the gdstore with a
//	target that matches the digest provided in the refOrID, but none are an image name match
//	to refOrID.
//
// 3: error
//
//	An error looking up refOrID or no gdstore found with matching name or target. Note that the first
...
func (i *ImageService) resolveAllReferences(ctx context.Context, refOrID string) (*containerdimages.Image, []containerdimages.Image, error) {
	...
	if idWithoutAlgo := checkTruncatedID(refOrID); idWithoutAlgo != "" { // Valid ID.
		...
		if err != nil {
			...
			// to match across gdstore with the matching target.
			if dgst == "" {
				return nil, nil, gdstore.ErrImageDoesNotExist{Ref: parsed}
			}
		} else {
	...
	// Lookup up all associated gdstore and check for consistency with first reference
