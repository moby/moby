...
import (
	...
	"github.com/docker/docker/daemon/imagestore/gdstore"
...
// ExportImage exports a list of gdstore to the given output stream. The
// exported gdstore are archived into a tar when written to the output
// stream. All gdstore with the given tag and all versions containing
// the same tag are exported. names is the set of tags to export, and
// outStream is the writer which the gdstore are written to.
//
...
func (i *ImageService) ExportImage(ctx context.Context, names []string, platform *ocispec.Platform, outStream io.Writer) error {
	...
	opts := []archive.ExportOpt{
		...
		// The c8dstore will choose only one manifest for the `manifest.json`.
		...
		// Note: This only matters when importing this archive into non-c8dstore Docker.
		// Importing the same archive into c8dstore, will not restrict the platforms.
		...
		if ref != nil {
			...
			for k, v := range orgTarget.Annotations {
				...
				case containerdimages.AnnotationImageName, ocispec.AnnotationRefName:
					...
					// Otherwise c8dstore will use it as name.
				default:
	...
	exportRepository := func(ctx context.Context, ref reference.Named) error {
		...
		if err != nil {
			return errdefs.System(fmt.Errorf("failed to list all gdstore from repository %s: %w", ref.Name(), err))
		}
	...
	for _, name := range names {
		...
		if resolveErr != nil || !specificDigestResolved {
			...
			if refErr == nil && reference.IsNameOnly(ref) {
				...
				// Export all gdstore with the same repository.
...
// LoadImage uploads a set of gdstore into the repository. This is the
// complement of ExportImage.  The input stream is an uncompressed tar
// ball containing gdstore and metadata.
func (i *ImageService) LoadImage(ctx context.Context, inTar io.ReadCloser, platform *ocispec.Platform, outStream io.Writer, quiet bool) error {
	...
	opts := []containerd.ImportOpt{
		...
		// Create an additional image with dangling name for imported gdstore...
	...
	if err != nil {
		if platform != nil {
			...
			log.G(ctx).WithFields(log.Fields{"error": err, "platform": p}).Debug("failed to import image to c8dstore")
			...
			// FilterPlatform c8dstore handler.
		...
		log.G(ctx).WithError(err).Debug("failed to import image to c8dstore")
		return errdefs.System(err)
	}

	if platform != nil {
		// Verify that the requested platform is available for the loaded gdstore.
...
func (i *ImageService) verifyImagesProvidePlatform(ctx context.Context, imgs []containerdimages.Image, platform ocispec.Platform, pm platforms.Matcher) error {
	if len(imgs) == 0 {
		return errdefs.NotFound(fmt.Errorf("no gdstore providing the requested platform %s found", platforms.FormatAll(platform)))
	}
	...
	case 0:
		// Success - All gdstore provide the requested platform.
		return nil
	case 1:
		msg = "image %s was loaded, but doesn't provide the requested platform (%s)"
	default:
		msg = "gdstore [%s] were loaded, but don't provide the requested platform (%s)"
	}
