...
import (
	...
	images2 "github.com/docker/docker/daemon/imagestore/gdstore"
...
//
// If prune is true, ancestor gdstore will each attempt to be deleted quietly,
...
func (i *ImageService) ImageDelete(ctx context.Context, imageRef string, force, prune bool) (response []imagetypes.DeleteResponse, retErr error) {
	...
	defer func() {
		if retErr == nil {
			images2.ImageActions.WithValues("delete").UpdateSince(start)
		}
	}()
	...
	if img == nil {
		if len(all) == 0 {
			...
			return nil, images2.ErrImageDoesNotExist{Ref: parsed}
		}
		...
		if len(sameRef) == 0 && named != nil {
			return nil, images2.ErrImageDoesNotExist{Ref: named}
		}
