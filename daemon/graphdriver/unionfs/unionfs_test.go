...
var (
	tmp = path.Join(os.TempDir(), "unionfs-tests", "unionfs")
)
...
func TestUnionfsString(t *testing.T) {
	d := newDriver(t)
	defer os.RemoveAll(tmp)

	if d.String() != "unionfs" {
		t.Fatalf("Expected unionfs got %s", d.String())
	}
}
...
func TestApplyDiff(t *testing.T) {
	...
	if _, err := d.ApplyDiff("3", "", diff); err != nil {
		t.Fatal(err)
	}
