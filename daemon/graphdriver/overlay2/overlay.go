...
func (d *Driver) create(id, parent string, opts *graphdriver.CreateOpts) (retErr error) {
	...
	if err := fswriter.AtomicWriteFile(path.Join(dir, "link"), []byte(lid), 0o644); err != nil {
		return err
	}
	...
	if err := fswriter.AtomicWriteFile(path.Join(d.dir(parent), "committed"), []byte{}, 0o600); err != nil {
		return err
	}
	...
	if lower != "" {
		if err := fswriter.AtomicWriteFile(path.Join(dir, lowerFile), []byte(lower), 0o644); err != nil {
			return err
		}
	}
