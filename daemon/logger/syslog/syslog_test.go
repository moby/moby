...
func TestBuildSyslogParamsBasicUDPConfig(t *testing.T) {
	...
	if _, _, _, _, _, _, err := buildSyslogParams(info); err != nil {
	...
	_, proto, address, facility, formatter, framer, err := buildSyslogParams(info)
	if err != nil {
		t.Fatalf("buildSyslogParams returned error: %v", err)
	}
	if proto != secureProto {
		t.Fatalf("expected proto %q, got %q", secureProto, proto)
	}
	if address != "1.2.3.4:6514" {
		t.Fatalf("expected address %q, got %q", "1.2.3.4:6514", address)
	}
	if facility != syslog.LOG_DAEMON {
		t.Fatalf("expected LOG_DAEMON facility, got %v", facility)
	}
	if !functionMatches(rfc5424formatterWithAppNameAsTag, formatter) {
		t.Fatalf("unexpected formatter for secure rfc5424 config: %#v", formatter)
	}
	if !functionMatches(syslog.RFC5425MessageLengthFramer, framer) {
		t.Fatalf("unexpected framer for secure rfc5424 config: %#v", framer)
	}
}
