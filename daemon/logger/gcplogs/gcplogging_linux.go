...
// ensureHomeIfIAmStatic ensure $HOME to be set if autoversion.IAmStatic is "true".
...
func ensureHomeIfIAmStatic() error {
	// Note: autoversion.IAmStatic and homedir.GetStatic() is only available for linux.
	// So we need to use them in this gcplogging_linux.go rather than in gcplogging.go
	if autoversion.IAmStatic == "true" && os.Getenv("HOME") == "" {
