...
func (sb *Sandbox) getGatewayEndpoint() (ep4, ep6 *Endpoint) {
	for _, ep := range sb.Endpoints() {
		...
		if gw4 && ep4 == nil {
			// Endpoints are already sorted by gateway priority, then by dual-stack
			// preference for equal priorities, then by name. The first endpoint
			// with IPv4 connectivity is the IPv4 gateway.
			ep4 = ep
		}
		if gw6 && ep6 == nil {
