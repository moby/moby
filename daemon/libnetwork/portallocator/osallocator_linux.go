...
func DetachSocketFilter(f *os.File) error {
	// SO_DETACH_FILTER requires a SockFprog to be specified, but it doesn't matter if it's empty.
	var prog unix.SockFprog

	return unix.SetsockoptSockFprog(int(f.Fd()), syscall.SOL_SOCKET, syscall.SO_DETACH_FILTER, &prog)
}
...
func setSocketFilter(sd int) error {
	...
	return unix.SetsockoptSockFprog(sd, syscall.SOL_SOCKET, syscall.SO_ATTACH_FILTER, &unix.SockFprog{
		Len:    uint16(len(asm)),
		Filter: (*unix.SockFilter)(unsafe.Pointer(&asm[0])), // #nosec G103 -- Ignore "G103: Use of unsafe calls should be audited"
	})
}
