...
//
// To modify the table, get a [Modifier] using [Table.Modifier], add commands
// to it, and call [Modifier.Apply].
//
...
// Modifier retrieves an object that can be used to manipulate the table.
func (t Table) Modifier() *Modifier {
	return &Modifier{t: t.t}
}
...
// IsValid returns true if tm is valid.
func (tm *Modifier) IsValid() bool {
	return tm.t != nil
}

// Name returns the name of the table tm will modify, or the
// empty string if tm is not valid.
func (tm *Modifier) Name() string {
	if !tm.IsValid() {
		return ""
	}
	return tm.t.Name
...
// Family returns the address family of the nftables table to be modified,
// or the empty string if tm is not valid.
func (tm *Modifier) Family() Family {
	if !tm.IsValid() {
		return ""
	}
	return tm.t.Family
...
func (tm *Modifier) Apply(ctx context.Context) (retErr error) {
	if !Enabled() {
		return errors.New("nftables is not enabled")
	}
	if !tm.IsValid() {
		return errors.New("table modifier is not valid")
	}
	tm.t.applyLock.Lock()
	defer tm.t.applyLock.Unlock()
	...
	defer func() {
		...
		slices.Reverse(rollback)
		for _, c := range rollback {
			if _, err := c.rollback(ctx, tm.t); err != nil {
	...
	for _, cmd := range tm.cmds {
		applied, err := cmd.apply(ctx, tm.t)
		if err != nil {
