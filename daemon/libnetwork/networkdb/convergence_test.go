...
// agreed returns a predicate reporting whether every node which has joined nw
// agrees that exactly the nodes in want participate, and holds the same table
// contents. It is a constructor rather than the check itself so that want is
// converted once, not on each of the many calls a polling loop makes.
//
...
// leaving. Presence alone is not enough, because LeaveNetwork keeps the entry
// around with leaving set for reapNetworkInterval while removing the node from
// networkNodes -- so a departing node would be held to a member set it has
// already left, and could never satisfy it.
