...
// Driver conforms to storage.Driver interface and
...
func newDriver(t *testing.T, name string) *Driver {
	root, err := ioutil.TempDir("", "docker-storagetest-")
	if err != nil {
		t.Fatal(err)
	}
	...
	d, err := storage.GetDriver(name, root, nil, nil, nil)
	if err != nil {
		t.Logf("storage: %v\n", err)
		if err == storage.ErrNotSupported || err == storage.ErrPrerequisites || err == storage.ErrIncompatibleFS {
