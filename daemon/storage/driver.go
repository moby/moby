...
var (
	...
	ErrIncompatibleFS = fmt.Errorf("backing file system is unsupported for this storage driver")
)
...
func GetDriver(name, home string, options []string, uidMaps, gidMaps []idtools.IDMap) (Driver, error) {
	...
	logrus.Errorf("Failed to GetDriver storage %s %s", name, home)
...
func getBuiltinDriver(name, home string, options []string, uidMaps, gidMaps []idtools.IDMap) (Driver, error) {
	...
	logrus.Errorf("Failed to built-in GetDriver storage %s %s", name, home)
	return nil, ErrNotSupported
}
...
func New(root string, name string, options []string, uidMaps, gidMaps []idtools.IDMap) (Driver, error) {
	if name != "" {
		logrus.Debugf("[storage] trying provided driver %q", name) // so the logs show specified driver
	...
	for _, name := range priority {
		...
		if _, prior := driversMap[name]; prior {
			...
			if err != nil {
				...
				logrus.Errorf("[storage] prior storage driver %q failed: %s", name, err)
				return nil, err
			}
			...
			if len(driversMap)-1 > 0 {
				...
				return nil, fmt.Errorf("%q contains several valid storages: %s; Please cleanup or explicitly choose storage driver (-s <DRIVER>)", root, strings.Join(driversSlice, ", "))
			}

			logrus.Infof("[storage] using prior storage driver %q", name)
			return driver, nil
		}
	}
...
// the storage  driver is a non-supported error.
