...
// If a overlay filesystem is not supported over a existing filesystem then error storage.ErrIncompatibleFS is returned.
func Init(home string, options []string, uidMaps, gidMaps []idtools.IDMap) (storage.Driver, error) {
	...
	switch fsMagic {
	case storage.FsMagicAufs, storage.FsMagicBtrfs, storage.FsMagicOverlay, storage.FsMagicZfs:
...
func (d *Driver) mounted(dir string) (bool, error) {
	return storage.Mounted(storage.FsMagicOverlay, dir)
}
