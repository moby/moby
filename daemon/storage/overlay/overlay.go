...
func NaiveDiffDriverWithApply(driver ApplyDiffProtoDriver, uidMaps, gidMaps []idtools.IDMap) storage.Driver {
	return &naiveDiffDriverWithApply{
		Driver:    storage.NewNaiveDiffDriver(driver, uidMaps, gidMaps),
		applyDiff: driver,
	}
}
...
// If overlay filesystem is not supported on the host, storage.ErrNotSupported is returned as error.
// If an overlay filesystem is not supported over an existing filesystem then error storage.ErrIncompatibleFS is returned.
func Init(home string, options []string, uidMaps, gidMaps []idtools.IDMap) (storage.Driver, error) {

	if err := supportsOverlay(); err != nil {
		return nil, storage.ErrNotSupported
	}

	fsMagic, err := storage.GetFSMagic(home)
	if err != nil {
		return nil, err
	}
	if fsName, ok := storage.FsNames[fsMagic]; ok {
		backingFs = fsName
	}

	switch fsMagic {
	case storage.FsMagicAufs, storage.FsMagicBtrfs, storage.FsMagicOverlay, storage.FsMagicZfs, storage.FsMagicEcryptfs:
	...
	d := &Driver{
		...
		uidMaps: uidMaps,
		gidMaps: gidMaps,
		ctr:     storage.NewRefCounter(storage.NewFsChecker(storage.FsMagicOverlay)),
	}
