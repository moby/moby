...
// init registers the windows storage drivers to the register.
func init() {
	storage.Register("windowsfilter", InitFilter)
	storage.Register("windowsdiff", InitDiff)
...
// Driver represents a windows storage driver.
type Driver struct {
	// info stores the shim driver information
	info hcsshim.DriverInfo
}

var _ storage.DiffGetterDriver = &Driver{}
...
func InitFilter(home string, options []string, uidMaps, gidMaps []idtools.IDMap) (storage.Driver, error) {
	logrus.Debugf("WindowsStorage InitFilter at %s", home)
	d := &Driver{
		info: hcsshim.DriverInfo{
			HomeDir: home,
...
func InitDiff(home string, options []string, uidMaps, gidMaps []idtools.IDMap) (storage.Driver, error) {
	logrus.Debugf("WindowsStorage InitDiff at %s", home)
	d := &Driver{
		info: hcsshim.DriverInfo{
			HomeDir: home,
			Flavour: diffDriver,
...
func (d *Driver) Get(id, mountLabel string) (string, error) {
	logrus.Debugf("WindowsStorage Get() id %s mountLabel %s", id, mountLabel)
	var dir string
...
func (d *Driver) Put(id string) error {
	logrus.Debugf("WindowsStorage Put() id %s", id)
...
func (d *Driver) ApplyDiff(id, parent string, diff archive.Reader) (int64, error) {
	if d.info.Flavour == diffDriver {
		start := time.Now().UTC()
		logrus.Debugf("WindowsStorage ApplyDiff: Start untar layer")
		...
		logrus.Debugf("WindowsStorage ApplyDiff: Untar time: %vs", time.Now().UTC().Sub(start).Seconds())
...
// importLayer adds a new layer to the tag and storage store based on the given data.
...
func (d *Driver) DiffGetter(id string) (storage.FileGetCloser, error) {
