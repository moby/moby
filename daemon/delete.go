...
func (daemon *Daemon) RemoveVolumes(volumes map[string]*Volume) {
	for _, v := range volumes {
		...
		volumeId := v.GetId()
		inUse, container := daemon.volumes.InUse(volumeId)
		if inUse {
			log.Infof("The volume %s is used by the container %s. Impossible to remove it. Skipping.", volumeId, container)
			continue
		}
		if err := daemon.Volumes().Delete(volumeId); err != nil {
			// Do not return here just because 1 volume couldn't be deleted
			// Just log the event
			log.Errorf("Error calling volumes.Delete(%q): %v", volumeId, err)
		}
	}
	return
}
