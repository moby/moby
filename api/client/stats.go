...
func (s *containerStats) Display(w io.Writer) error {
	...
	fmt.Fprintf(w, "%s\t%.2f%%\t%s / %s\t%.2f%%\t%s / %s\t%s / %s\t%s / %s\t%.2f / %.2f\n",
		s.Name,
		s.CPUPercentage,
		...
		s.BlockReadIOPS, s.BlockWriteIOPS)
	return nil
}
...
func (cli *DockerCli) CmdStats(args ...string) error {
	...
	printHeader := func() {
		...
		io.WriteString(w, "CONTAINER\tCPU %\tMEM USAGE / LIMIT\tMEM %\tNET I/O\tBIO(Bytes)\tBIO(Rate)\tBIO(IOPS)\n")
	}
	for _, n := range names {
