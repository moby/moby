...
func (cli *DockerCli) CmdBuild(args ...string) error {
	...
	file := cmd.String([]string{"f", "-file"}, "", "Build a new container image from the Dockerfile specified")
	...
	if cmd.Arg(0) == "-" {
		...
		if _, err = os.Stat(*file); os.IsNotExist(err) {
			return fmt.Errorf("no Dockerfile found in %s", *file)
		}
		dockerfile, err := ioutil.ReadFile(*file)
		if err != nil {
			return fmt.Errorf("Error trying to use file %s as Dockerfile", *file)
		}
		context, err = archive.Generate("Dockerfile", string(dockerfile))
		if err != nil {
			return fmt.Errorf("Error trying to use file %s as Dockerfile", *file)
