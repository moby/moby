...
func (cli *DockerCli) CmdBuild(args ...string) error {
	...
	*noCache = !*cmd.Bool([]string{"cache"}, true, "Usa cache when building the image")
...
func (cli *DockerCli) CmdRmi(args ...string) error {
	...
	*noprune = !*cmd.Bool([]string{"prune"}, true, "Delete untagged parents")
	...
	*trunc = !*cmd.Bool([]string{"#notrunc", "#-no-trunc"}, false, "Don't truncate output")
	...
	if *flViz || *flTree {
		...
		if startImage != nil {
			...
			root.Add(startImage)
			cli.WalkTree(*trunc, root, byParent, "", printNode)
		} else if filter == "" {
			cli.WalkTree(*trunc, roots, byParent, "", printNode)
		}
		if *flViz {
...
// FIXME: --viz and --tree are deprecated. Remove them in a future version.
func (cli *DockerCli) WalkTree(trunc bool, images *engine.Table, byParent map[string]*engine.Table, prefix string, printNode func(cli *DockerCli, noTrunc bool, image *engine.Env, prefix string)) {
	length := images.Len()
	if length > 1 {
		for index, image := range images.Data {
			if index+1 == length {
				printNode(cli, trunc, image, prefix+"└─")
				if subimages, exists := byParent[image.Get("Id")]; exists {
					cli.WalkTree(trunc, subimages, byParent, prefix+"  ", printNode)
				}
			} else {
				printNode(cli, trunc, image, prefix+"\u251C─")
				if subimages, exists := byParent[image.Get("Id")]; exists {
...
// FIXME: --viz and --tree are deprecated. Remove them in a future version.
func (cli *DockerCli) printVizNode(trunc bool, image *engine.Env, prefix string) {
	var (
		imageID  string
		parentID string
	)
...
// FIXME: --viz and --tree are deprecated. Remove them in a future version.
func (cli *DockerCli) printTreeNode(trunc bool, image *engine.Env, prefix string) {
	var imageID string
	if trunc {
		imageID = utils.TruncateID(image.Get("Id"))
	} else {
...
func (cli *DockerCli) CmdAttach(args ...string) error {
	...
	*stdin = !*cmd.Bool([]string{"#nostdin", "#-no-stdin"}, false, "Do not attach stdin")
	...
	v := url.Values{}
	v.Set("stream", "1")
	if *stdin && container.Config.OpenStdin {
		v.Set("stdin", "1")
		in = cli.in
	}
	v.Set("stdout", "1")
