...
func (cli *DockerCli) CmdDaemon(args ...string) error {
	...
	flEnableCors := cmd.Bool([]string{"#api-enable-cors", "-api-enable-cors"}, false, "Enable CORS headers in the remote API")
	flSocketGroup := cmd.String([]string{"G", "-group"}, "docker", "Group to assign the unix socket specified by -H when running in daemon mode\nuse '' (the empty string) to disable setting of a group")
	...
	if err := registry.NewService(config.InsecureRegistries).Install(eng); err != nil {
		log.Fatal(err)
	}
	...
	// Serve api
	job := eng.Job("serveapi", daemon.Hosts...)
	job.SetenvBool("Logging", true)
	job.SetenvBool("EnableCors", *flEnableCors)
	job.Setenv("Version", dockerversion.VERSION)
	job.Setenv("SocketGroup", *flSocketGroup)

	job.SetenvBool("Tls", daemon.Tls)
	job.SetenvBool("TlsVerify", daemon.TlsVerify)
	job.Setenv("TlsCa", daemon.Ca)
	job.Setenv("TlsCert", daemon.Cert)
	job.Setenv("TlsKey", daemon.Key)
