...
func (cli *DockerCli) CmdBuild(args ...string) error {
	...
	runconfig.AddRunConfigFlags(cmd)
	if err := cmd.Parse(args); err != nil {
		return nil
	}
	if cmd.NArg() != 1 {
	...
	runConfig, err := runconfig.ParseRunConfig(cmd)
	if err != nil {
		return err
	}
	...
	if runConfig != nil {
		buf, err := json.Marshal(runConfig)
		if err != nil {
			return err
		}
		headers.Set("X-Docker-RunConfig", base64.URLEncoding.EncodeToString(buf))
	}
...
func (cli *DockerCli) CmdStart(args ...string) error {
	...
	var encounteredError error
	c := &runconfig.HostAndRunConfig{nil, runConfig}

	for _, name := range cmd.Args() {
		_, _, err := readBody(cli.call("POST", "/containers/"+name+"/start", c, false))
		if err != nil {
			if !*attach || !*openStdin {
				fmt.Fprintf(cli.err, "%s\n", err)
			}
...
func (cli *DockerCli) CmdRun(args ...string) error {
	...
	runconfig.AddRunConfigFlags(cmd)
	config, hostConfig, err := runconfig.ParseSubcommand(cmd, args, nil)
	if err != nil {
		return err
	}
	...
	//start the container
	c := &runconfig.HostAndRunConfig{hostConfig, runConfig}

	if _, _, err = readBody(cli.call("POST", "/containers/"+runResult.Get("Id")+"/start", c, false)); err != nil {
		return err
	}
