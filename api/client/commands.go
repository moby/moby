...
func verifyMacroDefinition(value string) (string, error) {
	if strings.Index(value, "=") < 0 {
		return "", fmt.Errorf("invalid macro definition: %s", value)
	}
...
func (cli *DockerCli) CmdBuild(args ...string) error {
	...
	var (
		macroDefinitions = opts.NewListOpts(verifyMacroDefinition)
	...
	cmd.Var(&macroDefinitions, []string{"D", "-define"}, "Applies the macro definitions before executing the Dockerfile")
	...
	for _, definition := range macroDefinitions.GetAll() {
		item := strings.SplitN(definition, "=", 2)

		identifier := item[0]
		value := item[1]

		macros[identifier] = value
	}
