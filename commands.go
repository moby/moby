...
func (cli *DockerCli) CmdRun(args ...string) error {
	...
	if imgValues.Config.Param != nil {
		imgParam, confParam := decodeParams(imgValues.Config.Param), decodeParams(config.Param)
		for k, kv := range imgParam {
			if _, ok := confParam[k]; !ok {
				fmt.Fprintf(cli.out, "Missing required parameter %s (enter for default: %s): ", k, kv)
