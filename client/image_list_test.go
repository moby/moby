...
func TestImageListError(t *testing.T) {
	...
	_, err := client.ImageList(context.Background(), ListOptions{})
...
func TestImageListWithSharedSize(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "set after 1.42, if requested", version: "1.42", options: ListOptions{SharedSize: true}, sharedSize: "1"},
		{name: "unset before 1.42, even if requested", version: "1.41", options: ListOptions{SharedSize: true}},
	} {
