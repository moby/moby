...
func (cli *Client) doRequest(req *http.Request) (serverResponse, error) {
	...
	if err != nil {
		...
		if cli.scheme == "https" && strings.Contains(err.Error(), "bad certificate") {
			return serverResp, errConnectionFailed{pkgError.Wrap(err, "the server probably has client authentication (--tlsverify) enabled; check your TLS client certification settings")}
		}
		...
		if uErr, ok := err.(*url.Error); ok {
			if nErr, ok := uErr.Err.(*net.OpError); ok {
				if os.IsPermission(nErr.Err) {
					return serverResp, errConnectionFailed{pkgError.Wrapf(err, "permission denied while trying to connect to the Docker daemon socket at %v", cli.host)}
				}
			}
		}
		...
		if strings.Contains(err.Error(), `open //./pipe/docker_engine`) {
			...
			if f, elevatedErr := os.Open(`\\.\PHYSICALDRIVE0`); elevatedErr != nil {
				err = pkgError.Wrap(err, "in the default daemon configuration on Windows, the docker client must be run with elevated privileges to connect")
			} else {
				_ = f.Close()
				err = pkgError.Wrap(err, "this error may indicate that the docker daemon is not running")
			}
		}

		return serverResp, errConnectionFailed{pkgError.Wrap(err, "error during connect")}
	}
