...
// signature of ImageLoadWithOpts.
// TODO: Change to ImageLoad(ctx context.Context, input io.Reader, opts image.LoadOptions) (image.LoadResponse, error) {
func (cli *Client) ImageLoad(ctx context.Context, input io.Reader, quiet bool) (image.LoadResponse, error) {
	return cli.ImageLoadWithOpts(ctx, input, image.LoadOptions{
		Quiet: quiet,
	})
}

// ImageLoadWithOpts loads an image in the docker host from the client host.
// It's up to the caller to close the io.ReadCloser in the
// ImageLoadResponse returned by this function.
//
...
//
// Note: This function will be renamed to ImageLoad in future versions.
func (cli *Client) ImageLoadWithOpts(ctx context.Context, input io.Reader, opts image.LoadOptions) (image.LoadResponse, error) {
	v := url.Values{}
	v.Set("quiet", "0")
	if opts.Quiet {
		v.Set("quiet", "1")
