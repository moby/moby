...
func TestImageInspectWithManifestsAndPlatform(t *testing.T) {
	client := &Client{
		client: newMockClient(func(req *http.Request) (*http.Response, error) {
			return nil, fmt.Errorf("should not make request")
		}),
	}

	_, err := client.ImageInspect(context.Background(), "image_id",
		ImageInspectWithManifests(true),
		ImageInspectWithPlatform(&ocispec.Platform{
			OS:           "linux",
			Architecture: "arm64",
		}),
	)
	assert.Check(t, is.ErrorContains(err, "manifests and platform options cannot both be set"))
}
