...
func TestGetAPIPath(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: "/v" + versions.DefaultVersion + "/containers/json",
		},
		{
			path:     "/containers/json",
			query:    url.Values{},
		...
		},
		{
			...
			expected: "/v" + versions.DefaultVersion + "/containers/json?s=c",
		},
		{
			version:  "1.22",
			path:     "/containers/json",
...
func TestNewClientWithOpsFromEnvSetsDefaultVersion(t *testing.T) {
	...
	assert.Check(t, is.Equal(client.ClientVersion(), versions.DefaultVersion))
...
func TestNegotiateAPIVersionAutomatic(t *testing.T) {
	...
	expected := versions.DefaultVersion
	assert.Equal(t, client.ClientVersion(), expected)
