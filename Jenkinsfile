...
pipeline {
    ...
    parameters {
        ...
        booleanParam(name: 'windows2022hyperv', defaultValue: true, description: 'Windows 2022 (LTSC) with Hyper-V isolation Build/Test')
    ...
    stages {
        ...
        stage('Build') {
            parallel {
                ...
                stage('win-2022-hyperv') {
                    when {
                        beforeAgent true
                        expression { params.windows2022hyperv }
                    }
                    environment {
                        DOCKER_BUILDKIT        = '0'
                        DOCKER_DUT_DEBUG       = '1'
                    ...
                    post {
                        always {
                            ...
                            catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE', message: 'Failed to create bundles.zip') {
                                powershell '''
                                cd $env:WORKSPACE
                                $bundleName="win-2022-hyperv-integration"
                                Write-Host -ForegroundColor Green "Creating ${bundleName}-bundles.zip"
