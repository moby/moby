...
begin:
	...
	name := strconv.Itoa(rand.Intn(len(left))) + "_" + strconv.Itoa(rand.Intn(len(right))) // G404: Use of weak random number generator (math/rand instead of crypto/rand)
	if name == "boring_wozniak" /* Steve Wozniak is not boring */ {
		goto begin
	}

	if retry > 0 {
		name += strconv.Itoa(rand.Intn(10)) // G404: Use of weak random number generator (math/rand instead of crypto/rand)
	}
