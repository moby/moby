...
func TestTarWithBlockCharFifo(t *testing.T) {
	...
	err = unix.Mknod(filepath.Join(origin, "2"), unix.S_IFBLK, int(unix.Mkdev(uint32(12), uint32(5))))
	assert.NilError(t, err)
	err = unix.Mknod(filepath.Join(origin, "3"), unix.S_IFCHR, int(unix.Mkdev(uint32(12), uint32(5))))
	assert.NilError(t, err)
	err = unix.Mknod(filepath.Join(origin, "4"), unix.S_IFIFO, int(unix.Mkdev(uint32(12), uint32(5))))
	assert.NilError(t, err)
