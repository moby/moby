...
// TarResourceReplaceBase() should change the name of the items
...
func TestCopyFromBaseChange(t *testing.T) {
	...
	srcInfo := CopyInfo{Path: srcDir, Exists: true, IsDir: true}

	content, err := TarResourceReplaceBase(srcDir, differentDirname)
	if err != nil {
		t.Fatalf("unexpected error %T: %s", err, err)
	}
	defer content.Close()
	...
	if err = CopyTo(content, srcInfo, tmpDirB); err != nil {
		t.Fatalf("unexpected error %T: %s", err, err)
	}
	...
	srcInfo = CopyInfo{Path: srcPath, Exists: true, IsDir: false}

	content, err = TarResourceReplaceBase(srcPath, differentFilename)
	if err != nil {
