...
func testCopyHelper(t *testing.T, srcPath, dstPath string) (err error) {
	...
	if content, err = CopyFrom(srcPath); err != nil {
		return
	}
	defer content.Close()

	err = CopyTo(content, dstPath)
...
func TestCopyErrSrcNotExists(t *testing.T) {
	...
	content, err := CopyFrom(filepath.Join(tmpDirA, "file1"))
	if err == nil {
		content.Close()
...
func TestCopyErrSrcNotDir(t *testing.T) {
	...
	content, err := CopyFrom(joinTrailingSep(tmpDirA, "file1"))
	if err == nil {
		content.Close()
...
func TestCopyErrDstParentNotExists(t *testing.T) {
	...
	if err = CopyTo(content, filepath.Join(tmpDirB, "fakeParentDir", "file1")); err == nil {
		t.Fatal("expected IsNotExist error, but got nil instead")
	}
	...
	content, err = CopyFrom(filepath.Join(tmpDirA, "dir1"))
	if err != nil {
		t.Fatalf("unexpected error %T: %s", err, err)
	}
	defer content.Close()
	...
	if err = CopyTo(content, joinTrailingSep(tmpDirB, "fakeParentDir", "fakeDstDir")); err == nil {
		t.Fatal("expected IsNotExist error, but got nil instead")
	}
...
func TestCopyErrDstNotDir(t *testing.T) {
	...
	if err = CopyTo(content, joinTrailingSep(tmpDirB, "file1")); err == nil {
		t.Fatal("expected IsNotDir error, but got nil instead")
	}
...
// CopyFromReplaceBase() should change the name of the items
// which it archives. Test that this behaves as intended.
func TestCopyFromBaseChange(t *testing.T) {
	tmpDirA, tmpDirB := getTestTempDirs(t)
	defer removeAllPaths(tmpDirA, tmpDirB)
	...
	differentDirname := "differentDirname"

	content, err := CopyFromReplaceBase(srcDir, differentDirname)
	if err != nil {
		t.Fatalf("unexpected error %T: %s", err, err)
	}
	defer content.Close()

	resultDir := filepath.Join(tmpDirB, differentDirname)

	if err = CopyTo(content, tmpDirB); err != nil {
		t.Fatalf("unexpected error %T: %s", err, err)
	}
	...
	// Also test with a filename.

	srcPath := filepath.Join(tmpDirA, "file1")
	differentFilename := "differentFilename"

	content, err = CopyFromReplaceBase(srcPath, differentFilename)
	if err != nil {
		t.Fatal("unable to copy from %q %T: %s", srcPath, err, err)
	}
	defer content.Close()

	dstPath := filepath.Join(tmpDirB, differentFilename)

	if err = CopyTo(content, tmpDirB); err != nil {
		t.Fatalf("unable to copy to %q %T: %s", dstPath, err, err)
	}
