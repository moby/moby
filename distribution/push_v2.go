...
func (pd *v2PushDescriptor) Upload(ctx context.Context, progressOutput progress.Output, parallelGzip bool) (distribution.Descriptor, error) {
	...
	// upload the blob
	desc, err := pd.uploadUsingSession(ctx, progressOutput, diffID, layerUpload, parallelGzip)
	if err != nil {
		return desc, err
	}
...
func (pd *v2PushDescriptor) uploadUsingSession(
	...
	case schema2.MediaTypeUncompressedLayer:
		compressedReader, compressionDone := compress(reader, parallelGzip)
