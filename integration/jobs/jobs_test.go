...
func TestJobLifecycle(t *testing.T) {
	...
	assert.NilError(t, err)
	run := waited.Run
	assert.Check(t, is.Equal(run.State, jobsv0.RunStateSucceeded), "run error: %q", run.Error)
...
func TestJobScheduleFires(t *testing.T) {
	...
	assert.NilError(t, err)
	assert.Check(t, is.Equal(waited.Run.State, jobsv0.RunStateSucceeded), "run error: %q", waited.Run.Error)
...
func TestJobTimeout(t *testing.T) {
	...
	assert.NilError(t, err)
	assert.Check(t, is.Equal(waited.Run.State, jobsv0.RunStateTimedOut), "run error: %q", waited.Run.Error)
}
