...
func pollForHealthStatusSummary(ctx context.Context, client client.APIClient, containerID string, healthStatus containertypes.HealthStatus) func(log poll.LogT) poll.Result {
	return func(log poll.LogT) poll.Result {
		...
		for _, container := range containers {
			if container.Health == nil && versions.LessThan(version, "1.52") {
				total++
			} else if container.Health != nil && container.Health.Status == healthStatus && versions.GreaterThanOrEqualTo(version, "1.52") {
				total++
			}
		}
