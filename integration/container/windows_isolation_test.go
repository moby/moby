...
func TestWindowsHyperVIsolation(t *testing.T) {
	...
	testcases := []struct {
	...
	}{
		{
			...
			skipIfNotContainerd: true, // Hyper-V isolation better supported with containerd runtime
			validate: func(t *testing.T, ctx context.Context, id string) {
				// Verify container is running
				ctrInfo := container.Inspect(ctx, t, apiClient, id)
				assert.Check(t, is.Equal(ctrInfo.State.Running, true))
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.skipIfNotContainerd && !testEnv.RuntimeIsWindowsContainerd() {
				t.Skip("Hyper-V isolation requires containerd runtime")
			}
...
func TestWindowsIsolationComparison(t *testing.T) {
	...
	// Only test Hyper-V if containerd runtime is available
	if testEnv.RuntimeIsWindowsContainerd() {
...
func TestWindowsProcessIsolationResourceConstraints(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Special handling for CPU count test case
			if tc.name == "CPU count constraint" {
				opts = append(opts, func(config *container.TestContainerConfig) {
					config.HostConfig.CPUCount = 2 // Limit container to 2 CPUs
				})
			}
