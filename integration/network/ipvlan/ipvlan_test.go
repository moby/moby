...
func testIpvlanL2MultiSubnet(t *testing.T, ctx context.Context, client dclient.APIClient, parent string) {
	...
	if parent == "" {
		...
		assert.Check(t, is.Equal(c1.NetworkSettings.Networks[netName].Gateway, netip.MustParseAddr("172.28.200.1")))
		// Inspect the v6 gateway to ensure the proper default GW was assigned
		assert.Check(t, is.Equal(c1.NetworkSettings.Networks[netName].IPv6Gateway, netip.MustParseAddr("2001:db8:abc8::1")))
	}
