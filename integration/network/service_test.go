...
func TestCustomIfnameIsPreservedOnLiveRestore(t *testing.T) {
	...
	_, err = apiClient.NetworkDisconnect(ctx, "bridge", ctrId, client.NetworkDisconnectOptions{Force: true})
	assert.NilError(t, err)
}
...
func TestCustomIfnameCollidesWithExistingIface(t *testing.T) {
	...
	_, err := apiClient.NetworkConnect(ctx, testnet, ctrId, client.NetworkConnectOptions{
...
func TestCustomIfnameWithMatchingDynamicPrefix(t *testing.T) {
	...
	_, err = apiClient.NetworkDisconnect(ctx, "testnet1", ctrId, client.NetworkDisconnectOptions{Force: false})
	assert.NilError(t, err)
	_, err = apiClient.NetworkDisconnect(ctx, "testnet2", ctrId, client.NetworkDisconnectOptions{Force: false})
	assert.NilError(t, err)
