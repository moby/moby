...
func TestPreferredSubnetRestore(t *testing.T) {
	...
	assert.Check(t, is.Len(v4Insp.IPAM.Config, 1))
	v4allocCidr := v4Insp.IPAM.Config[0].Subnet
	...
	assert.Check(t, is.Len(v6Insp.IPAM.Config, 1))
	v6allocCidr := v6Insp.IPAM.Config[0].Subnet
	...
	assert.Check(t, is.Len(dualStackInsp.IPAM.Config, 2))
	var dualv4, dualv6 netip.Prefix
	if dualStackInsp.IPAM.Config[0].Subnet.Addr().Is4() {
		dualv4 = dualStackInsp.IPAM.Config[0].Subnet
		dualv6 = dualStackInsp.IPAM.Config[1].Subnet
	} else {
		dualv4 = dualStackInsp.IPAM.Config[1].Subnet
		dualv6 = dualStackInsp.IPAM.Config[0].Subnet
	}
	...
	v4Insp = network.InspectNoError(ctx, t, c, v4netName, inspOpts)
	assert.Check(t, is.Len(v4Insp.IPAM.Config, 1))
	assert.Check(t, is.Equal(v4Insp.IPAM.Config[0].Subnet, v4allocCidr))

	v6Insp = network.InspectNoError(ctx, t, c, v6netName, inspOpts)
	assert.Check(t, is.Len(v6Insp.IPAM.Config, 1))
	assert.Check(t, is.Equal(v6Insp.IPAM.Config[0].Subnet, v6allocCidr))

	dualStackInsp = network.InspectNoError(ctx, t, c, dualStackNetName, inspOpts)
	assert.Check(t, is.Len(dualStackInsp.IPAM.Config, 2))
	var dualv4after, dualv6after netip.Prefix
	if dualStackInsp.IPAM.Config[0].Subnet.Addr().Is4() {
		dualv4after = dualStackInsp.IPAM.Config[0].Subnet
		dualv6after = dualStackInsp.IPAM.Config[1].Subnet
	} else {
		dualv4after = dualStackInsp.IPAM.Config[1].Subnet
		dualv6after = dualStackInsp.IPAM.Config[0].Subnet
	}
