...
func TestPublishedPortAlreadyInUse(t *testing.T) {
	...
	ctr1 := ctr.Run(ctx, t, apiClient,
		...
		ctr.WithPortMap(map[containertypes.PortRangeProto][]containertypes.PortBinding{"80/tcp": {{HostPort: "8000"}}}))
...
func TestAllPortMappingsAreReturned(t *testing.T) {
	...
	assert.DeepEqual(t, inspect.NetworkSettings.Ports, map[containertypes.PortRangeProto][]containertypes.PortBinding{
		"80/tcp": {
...
func TestPortMappingRestore(t *testing.T) {
	...
	cid := ctr.Run(ctx, t, c,
		ctr.WithExposedPorts("80/tcp"),
		ctr.WithPortMap(map[containertypes.PortRangeProto][]containertypes.PortBinding{"80/tcp": {}}),
		ctr.WithName(svrName),
