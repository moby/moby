	...
	for _, nw := range section.networks {
		for _, ctr := range nw.containers {
			...
			poll.WaitOn(t, func(_ poll.LogT) poll.Result {
				...
				assert.NilError(t, err)
				if len(cl) != 1 {
					t.Logf("%v: got %d containers, want 1", time.Now(), len(cl))
					return poll.Continue("got %d containers, want 1", len(cl))
				}
				// The DOCKER-INGRESS chain seems to be populated a few milliseconds
				// after the container starts, making this test flaky. So, also wait
				// for a conntrack "RELATED" rule to appear in the chain.
				// TODO(robmry) - is there something better to poll?
				ic := host.Run(t, "iptables", "-L", "DOCKER-INGRESS")
				t.Logf("%v: iptables -L DOCKER-INGRESS: %s", time.Now(), ic)
				if !strings.Contains(ic, "RELATED") {
					t.Logf("%v: ingress chain not ready, got: %s", time.Now(), ic)
					return poll.Continue("ingress chain not ready, got: %s", ic)
				}
				return poll.Success()
