...
func TestHostIPv4BridgeLabel(t *testing.T) {
	...
	if testEnv.FirewallBackendDriver() == "nftables" {
		chain := testutil.RunCommand(ctx, "nft", "list", "chain", "ip", "docker-bridges", "nat-postrouting-out__hostIPv4Bridge").Combined()
		exp := fmt.Sprintf(`oifname != "hostIPv4Bridge" ip saddr %s counter packets 0 bytes 0 snat to %s comment "SNAT"`,
