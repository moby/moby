...
func TestConnectWithPriority(t *testing.T) {
	...
	_, err := apiClient.NetworkConnect(ctx, "testnet5", ctrID, client.NetworkConnectOptions{
	...
	_, err = apiClient.NetworkConnect(ctx, "testnet2", ctrID, client.NetworkConnectOptions{
		EndpointConfig: &networktypes.EndpointSettings{GwPriority: 100},
	})
	assert.NilError(t, err)
	...
	_, err = apiClient.NetworkConnect(ctx, "testnet3", ctrID, client.NetworkConnectOptions{
	...
	_, err = apiClient.NetworkConnect(ctx, "testnet4", ctrID, client.NetworkConnectOptions{
		EndpointConfig: &networktypes.EndpointSettings{GwPriority: 10},
	})
	assert.NilError(t, err)
	...
	_, err = apiClient.NetworkDisconnect(ctx, "testnet2", ctrID, client.NetworkDisconnectOptions{Force: true})
	assert.NilError(t, err)
	...
	_, err = apiClient.NetworkDisconnect(ctx, "testnet3", ctrID, client.NetworkDisconnectOptions{Force: true})
	assert.NilError(t, err)
	...
	_, err = apiClient.NetworkDisconnect(ctx, "testnet4", ctrID, client.NetworkDisconnectOptions{Force: true})
	assert.NilError(t, err)
	checkCtrRoutes(t, ctx, apiClient, ctrID, syscall.AF_INET, 3, "default via 10.100.10.1 dev eth0")
	checkCtrRoutes(t, ctx, apiClient, ctrID, syscall.AF_INET6, 7, "default via fddd:4901:f594::1 dev eth0")
}
...
func TestMixL3IPVlanAndBridge(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			c.NetworkDisconnect(ctx, br46NetName, ctrId, client.NetworkDisconnectOptions{Force: false})
			...
			c.NetworkDisconnect(ctx, ipvNetName, ctrId, client.NetworkDisconnectOptions{Force: false})
			...
			c.NetworkConnect(ctx, br46NetName, ctrId, client.NetworkConnectOptions{
