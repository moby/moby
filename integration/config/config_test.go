...
func TestConfigsUpdate(t *testing.T) {
	...
	result, err := c.ConfigInspect(ctx, configID, client.ConfigInspectOptions{})
	assert.NilError(t, err)
	assert.Check(t, is.Equal(result.Config.ID, configID))

	// test UpdateConfig with full ID
	result.Config.Spec.Labels = map[string]string{"test": "test1"}
	err = c.ConfigUpdate(ctx, configID, client.ConfigUpdateOptions{
		Version: result.Config.Version,
		Config:  result.Config.Spec,
	})
	assert.NilError(t, err)

	result, err = c.ConfigInspect(ctx, configID, client.ConfigInspectOptions{})
	assert.NilError(t, err)
	assert.Check(t, is.Equal(result.Config.Spec.Labels["test"], "test1"))

	// test UpdateConfig with full name
	result.Config.Spec.Labels = map[string]string{"test": "test2"}
	err = c.ConfigUpdate(ctx, testName, client.ConfigUpdateOptions{
		Version: result.Config.Version,
		Config:  result.Config.Spec,
	})
	assert.NilError(t, err)
	...
	result, err = c.ConfigInspect(ctx, configID, client.ConfigInspectOptions{})
	assert.NilError(t, err)
	assert.Check(t, is.Equal(result.Config.Spec.Labels["test"], "test2"))

	// test UpdateConfig with prefix ID
	result.Config.Spec.Labels = map[string]string{"test": "test3"}
	err = c.ConfigUpdate(ctx, configID[:1], client.ConfigUpdateOptions{
		Version: result.Config.Version,
		Config:  result.Config.Spec,
	})
	assert.NilError(t, err)
