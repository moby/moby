...
// TODO test an image with multiple layers and assert that only the requested layer is exported
func TestSaveLayerAndThenLoad(t *testing.T) {
	eng := NewTestEngine(t)
	defer mkRuntimeFromEngine(eng, t).Nuke()

	// save image
	r := httptest.NewRecorder()
	req, err := http.NewRequest("GET", "/layers/"+unitTestImageID+"/get", nil)
	if err != nil {
		t.Fatal(err)
	}
