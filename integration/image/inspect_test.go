...
func TestImageInspectWithPlatform(t *testing.T) {
	...
	imageID := specialimage.Load(ctx, t, apiClient, func(s string) (*ocispec.Index, error) {
		i, _, err := specialimage.MultiPlatform(s, s, []ocispec.Platform{nativePlatform, differentPlatform})
		return i, err
	})
