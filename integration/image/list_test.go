...
func TestImagesFilterUntil(t *testing.T) {
	...
	list, err := apiClient.ImageList(ctx, client.ListOptions{Filters: filter})
	assert.NilError(t, err)
...
func TestAPIImagesListSizeShared(t *testing.T) {
	...
	_, err := apiClient.ImageList(ctx, client.ListOptions{SharedSize: true})
	assert.NilError(t, err)
}
...
func TestAPIImagesListManifests(t *testing.T) {
	...
	t.Run("unsupported before 1.47", func(t *testing.T) {
		...
		images, err := c.ImageList(ctx, client.ListOptions{Manifests: true})
		assert.NilError(t, err)
	...
	t.Run("no manifests if not requested", func(t *testing.T) {
		images, err := api147.ImageList(ctx, client.ListOptions{})
		assert.NilError(t, err)
	...
	images, err := api147.ImageList(ctx, client.ListOptions{Manifests: true})
	assert.NilError(t, err)
