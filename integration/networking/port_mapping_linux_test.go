		...
		t.Run(tc.name, func(t *testing.T) {
			...
			t.Run("probes", func(t *testing.T) {
				for _, fam := range []struct {
					...
					// IPv6, this also needs Neighbour Discovery between the two
					// containers, so that the server can resolve the client's
					// link-layer address to reply.
...
// error signalling, including PMTUD, would silently break on this path.
//
...
// port closed only if that error is delivered. If the error were dropped, nc
// would see nothing and report the port open, as it does for a second
// container that has a listener.
//
// The two userland-proxy settings behave differently here, so each is set
// explicitly and checked separately. With docker-proxy in the path, the flow
// ends in the host's netns and the proxy forwards it on its own socket. The
// container's error goes back to the proxy instead of the peer, so the peer
// sees nothing and reads the port as open. That is a property of the userland
// proxy, not of these rules, but the test checks it too.
//
// Regression test for https://github.com/moby/moby/issues/53713
...
func testICMPErrorsFromPublishedPortICCDisabled(t *testing.T, userlandProxy bool) {
	...
	// Everything below is torn down with defer, not t.Cleanup, because
	// t.Cleanup runs after every defer, including the one that stops the daemon.
	defer network.RemoveNoError(ctx, t, c, netName)

	// Two servers, both publishing udp/5000: one with nothing listening on it,
	// and one with a listener.
	...
	// Checked before the probes, so that a setup problem fails the test rather
	// than showing up as an unreachable port.
	assert.Assert(t, openPorts[true] != "",
		"the listening server's port wasn't published (bindings: %v)", openPorts)
	assert.Assert(t, closedPorts[true] != "" && closedPorts[false] != "",
		"the closed server's port wasn't published for both families (bindings: %v)", closedPorts)

	// Every probe waits out nc's -w timeout, whether or not an error comes back,
	// so run them in parallel and wait out one timeout instead of three. t.Run
	// returns only once its parallel subtests have finished, so the deferred
	// cleanup above still runs after them.
	t.Run("probes", func(t *testing.T) {
		// Check that the probe can tell the two cases apart. With a listener
		// behind the published port, no error is generated and the port reads
		// as open. With docker-proxy the closed port reads as open too, so this
		// only tells them apart without the proxy.
		//
		// It's only done once, over IPv4. "nc -lu" serves a single datagram and
		// exits, so a probe per family would race with the loop restarting it,
		// and whether the probe can tell open from closed doesn't depend on the
		// address family.
		t.Run("listening-control", func(t *testing.T) {
			t.Parallel()
		...
		// both address families. If it were dropped, the probe would see nothing
		// and report the port open, as above.
		for _, fam := range []struct {
			name     string
			hostAddr string
			is4      bool
			...
			t.Run(fam.name+"/nothing-listening", func(t *testing.T) {
				t.Parallel()
				exitCode, stderr := scan(t, fam.hostAddr, closedPorts[fam.is4])
				if !userlandProxy {
					assert.Check(t, exitCode != 0,
						"expected the ICMP error to be delivered, so the port reads as closed (stderr: %s)", stderr)
					assert.Check(t, !strings.Contains(stderr, "open"),
						"port reported open, so no ICMP error was delivered: %s", stderr)
				} else {
					assert.Check(t, exitCode == 0 && strings.Contains(stderr, "open"),
						"docker-proxy terminates the flow, so the peer should see no error; got exit=%d stderr: %s",
						exitCode, stderr)
				}
			})
		}
