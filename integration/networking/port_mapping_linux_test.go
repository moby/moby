...
func TestPortMappedHairpinTCP(t *testing.T) {
	...
	serverId := container.Run(ctx, t, c,
		...
		container.WithPortMap(containertypes.PortMap{"80": {{HostIP: netip.MustParseAddr("0.0.0.0")}}}),
...
func TestPortMappedHairpinUDP(t *testing.T) {
	...
	serverId := container.Run(ctx, t, c,
		...
		container.WithPortMap(containertypes.PortMap{"54/udp": {{HostIP: netip.MustParseAddr("0.0.0.0")}}}),
...
func TestProxy4To6(t *testing.T) {
	...
	serverId := container.Run(ctx, t, c,
		container.WithNetworkMode(netName),
		container.WithExposedPorts("80"),
		container.WithPortMap(containertypes.PortMap{"80": {{HostIP: netip.MustParseAddr("::1")}}}),
		container.WithCmd("httpd", "-f"),
	)
	defer c.ContainerRemove(ctx, serverId, client.ContainerRemoveOptions{Force: true})
...
func TestAccessPortPublishedOnLoopbackAddress(t *testing.T) {
	...
	test := func(t *testing.T, host networking.Host, payload string) bool {
		...
		serverID := container.Run(ctx, t, c,
			...
			container.WithPortMap(containertypes.PortMap{"5000/udp": {{HostIP: netip.MustParseAddr(loIP), HostPort: hostPort}}}),
			container.WithNetworkMode(bridgeName))
		defer c.ContainerRemove(ctx, serverID, client.ContainerRemoveOptions{Force: true})
		...
		t.Run(proto, func(t *testing.T) {
			...
			ctrId := container.Run(ctx, t, c,
				...
				container.WithPortMap(containertypes.PortMap{
					...
					containertypes.PortRangeProto("80/" + proto): {{HostIP: netip.MustParseAddr("127.0.0.1")}},
				}),
			)
			defer c.ContainerRemove(ctx, ctrId, client.ContainerRemoveOptions{Force: true})
