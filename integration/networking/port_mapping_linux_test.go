...
func TestDisableNAT(t *testing.T) {
	...
	testcases := []struct {
		...
		gwMode4    string
		gwMode6    string
		expPortMap map[containertypes.PortRangeProto][]containertypes.PortBinding
	}{
		{
			name: "defaults",
			expPortMap: map[containertypes.PortRangeProto][]containertypes.PortBinding{
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			id := container.Run(ctx, t, c,
				...
				container.WithPortMap(map[containertypes.PortRangeProto][]containertypes.PortBinding{"80/tcp": {{HostPort: "8080"}}}),
			)
...
func TestPortMappedHairpinUDP(t *testing.T) {
	...
	serverId := container.Run(ctx, t, c,
		...
		container.WithPortMap(map[containertypes.PortRangeProto][]containertypes.PortBinding{"54/udp": {{HostIP: "0.0.0.0"}}}),
...
func TestProxy4To6(t *testing.T) {
	...
	serverId := container.Run(ctx, t, c,
		container.WithNetworkMode(netName),
		container.WithExposedPorts("80"),
		container.WithPortMap(map[containertypes.PortRangeProto][]containertypes.PortBinding{"80": {{HostIP: "::1"}}}),
		container.WithCmd("httpd", "-f"),
	)
	defer c.ContainerRemove(ctx, serverId, containertypes.RemoveOptions{Force: true})
	...
	for tcID, tc := range testcases {
		t.Run(fmt.Sprintf("userland-proxy=%t/IPv6=%t", tc.ulpEnabled, tc.ipv6), func(t *testing.T) {
			...
			serverID := container.Run(ctx, t, c,
				...
				container.WithExposedPorts("80/tcp"),
				container.WithPortMap(map[containertypes.PortRangeProto][]containertypes.PortBinding{"80/tcp": {{HostPort: hostPort}}}),
				container.WithCmd("httpd", "-f"),
...
func TestRestartUserlandProxyUnder2MSL(t *testing.T) {
	...
	ctrOpts := []func(*container.TestContainerConfig){
		...
		container.WithExposedPorts("80/tcp"),
		container.WithPortMap(map[containertypes.PortRangeProto][]containertypes.PortBinding{"80/tcp": {{HostPort: "1780"}}}),
		container.WithCmd("httpd", "-f"),
		container.WithNetworkMode(netName),
	}
...
func TestDirectRoutingOpenPorts(t *testing.T) {
	...
	createNet := func(gwMode string) ctrDesc {
		...
		ctrId := container.Run(ctx, t, c,
			...
			container.WithExposedPorts("80/tcp"),
			container.WithPortMap(map[containertypes.PortRangeProto][]containertypes.PortBinding{"80/tcp": {}}),
		)
		t.Cleanup(func() {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			retryFlaky(t, 5, func(t *testing.T) is.Comparison {
				serverID := container.Run(ctx, t, c,
					...
					container.WithPortMap(map[containertypes.PortRangeProto][]containertypes.PortBinding{"5000/tcp": {{HostPort: "5000"}}}),
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			test := func(t *testing.T, host networking.Host, daddr, payload string) bool {
				serverID := container.Run(ctx, t, c,
					...
					container.WithPortMap(map[containertypes.PortRangeProto][]containertypes.PortBinding{"5000/udp": {{HostPort: hostPort}}}),
					container.WithNetworkMode(bridgeName),
...
func TestAccessPortPublishedOnLoopbackAddress(t *testing.T) {
	...
	test := func(t *testing.T, host networking.Host, payload string) bool {
		...
		serverID := container.Run(ctx, t, c,
			...
			container.WithPortMap(map[containertypes.PortRangeProto][]containertypes.PortBinding{"5000/udp": {{HostIP: loIP, HostPort: hostPort}}}),
			container.WithNetworkMode(bridgeName))
		defer c.ContainerRemove(ctx, serverID, containertypes.RemoveOptions{Force: true})
	...
	for _, proto := range []string{"tcp", "udp"} {
		t.Run(proto, func(t *testing.T) {
			...
			ctrId := container.Run(ctx, t, c,
				...
				container.WithPortMap(map[containertypes.PortRangeProto][]containertypes.PortBinding{
