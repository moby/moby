...
func TestEtcHostsDisconnect(t *testing.T) {
	...
	_, err = c.NetworkConnect(ctx, netName2, ctrName, client.NetworkConnectOptions{})
	assert.Check(t, err)
	golden.Assert(t, getEtcHosts(), "TestEtcHostsDisconnect1.golden")

	// Disconnect net1, its hosts entries are currently before net2's.
	_, err = c.NetworkDisconnect(ctx, netName1, ctrName, client.NetworkDisconnectOptions{Force: false})
	assert.Check(t, err)
	golden.Assert(t, getEtcHosts(), "TestEtcHostsDisconnect2.golden")

	// Reconnect net1, so that its entries will follow net2's.
	_, err = c.NetworkConnect(ctx, netName1, ctrName, client.NetworkConnectOptions{})
	assert.Check(t, err)
	...
	_, err = c.NetworkDisconnect(ctx, netName2, ctrName, client.NetworkDisconnectOptions{Force: false})
	assert.Check(t, err)
