	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			res := container.RunAttach(ctx, t, c,
				...
				container.WithExtraHost("thehost:host-gateway"),
				container.WithCmd("wget", "-O-", "-T3", "http://thehost:8080"),
			)
			...
			if tc.expResponse {
				...
				assert.Check(t, is.Contains(res.Stderr.String(), "download timed out"))
			}
		})
	}
}
