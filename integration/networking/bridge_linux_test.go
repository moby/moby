...
func TestAdvertiseAddresses(t *testing.T) {
	...
	testcases := []struct {
		...
		},
		{
			...
			expNetCreateErr: `Error response from daemon: value for option com.docker.network.advertise_addr_ms "nonsense" must be an integer`,
		},
		{
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			checkPkts := func(pktDesc string, pkts []network.TimestampedPkt, matchIP netip.Addr, unpack func(pkt network.TimestampedPkt) (sh net.HardwareAddr, sp netip.Addr, err error)) {
				...
				assert.Check(t, is.Equal(count, tc.expNMsgs), "ARP message count")
			}
