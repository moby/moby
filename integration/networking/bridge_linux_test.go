...
func TestAdvertiseAddresses(t *testing.T) {
	...
	testcases := []struct {
		...
		},
		{
			name: "min interval",
			netOpts: []func(*networktypes.CreateOptions){
				network.WithOption(netlabel.AdvertiseAddrInterval, "100ms"),
			},
			expNMsgs:    3,
		...
		},
		{
			name: "cancel",
			netOpts: []func(*networktypes.CreateOptions){
				network.WithOption(netlabel.AdvertiseAddrInterval, "2s"),
			},
		...
		},
		{
			...
			netOpts: []func(*networktypes.CreateOptions){
				network.WithOption(netlabel.AdvertiseAddrInterval, "99ms"),
			},
			expNetCreateErr: "Error response from daemon: com.docker.network.advertise_addr_interval must be in the range 100ms to 2s",
		},
		{
			name: "interval too long",
			netOpts: []func(*networktypes.CreateOptions){
				network.WithOption(netlabel.AdvertiseAddrInterval, "2001ms"),
			},
			expNetCreateErr: "Error response from daemon: com.docker.network.advertise_addr_interval must be in the range 100ms to 2s",
		},
		{
			...
			netOpts: []func(*networktypes.CreateOptions){
				network.WithOption(netlabel.AdvertiseAddrInterval, "nonsense"),
			},
			expNetCreateErr: `Error response from daemon: value for option com.docker.network.advertise_addr_interval "nonsense" must be parseable by time.ParseDuration()`,
		},
		{
