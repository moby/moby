	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			ctrPubId := container.Run(ctx, t, c,
				...
				container.WithExposedPorts("80/tcp"),
				container.WithPortMap(map[containertypes.PortRangeProto][]containertypes.PortBinding{"80/tcp": {containertypes.PortBinding{HostPort: "8080"}}}),
				container.WithCmd("httpd", "-f"),
			)
...
func TestGatewaySelection(t *testing.T) {
	...
	ctrId := container.Run(ctx, t, c,
		...
		container.WithPortMap(map[containertypes.PortRangeProto][]containertypes.PortBinding{"80": {{HostPort: "8080"}}}),
		container.WithCmd("httpd", "-f"),
	)
	defer c.ContainerRemove(ctx, ctrId, containertypes.RemoveOptions{Force: true})
...
func TestDropInForwardChain(t *testing.T) {
	...
	l3.Hosts[l3SegHost].Do(t, func() {
		...
		ctrId := container.Run(ctx, t, c,
			...
			container.WithExposedPorts("80"),
			container.WithPortMap(map[containertypes.PortRangeProto][]containertypes.PortBinding{"80": {{HostPort: hostPort}}}),
			container.WithCmd("httpd", "-f"),
		)
		defer c.ContainerRemove(ctx, ctrId, containertypes.RemoveOptions{Force: true})
