...
func TestInternalNetworkDNS(t *testing.T) {
	...
	_, err = c.NetworkConnect(ctx, intNetName, ctrId, client.NetworkConnectOptions{})
	assert.NilError(t, err)
	res, err = container.Exec(ctx, c, ctrId, []string{"nslookup", "test.example"})
	assert.NilError(t, err)
	assert.Check(t, is.Equal(res.ExitCode, 0))
	...
	_, err = c.NetworkDisconnect(ctx, extNetName, ctrId, client.NetworkDisconnectOptions{Force: true})
	assert.NilError(t, err)
	res, err = container.Exec(ctx, c, ctrId, []string{"nslookup", "test.example"})
	assert.NilError(t, err)
	...
	_, err = c.NetworkConnect(ctx, extNetName, ctrId, client.NetworkConnectOptions{})
	assert.NilError(t, err)
	res, err = container.Exec(ctx, c, ctrId, []string{"nslookup", "test.example"})
	assert.NilError(t, err)
	assert.Check(t, is.Equal(res.ExitCode, 0))
