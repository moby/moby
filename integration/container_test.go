...
func TestContainerNetwork(t *testing.T) {
	...
	if container.State.GetExitCode() != 0 {
		t.Errorf("Unexpected ping 127.0.0.1 exit code %d (expected 0)", container.State.GetExitCode())
	}
}
...
func TestExitCode(t *testing.T) {
	...
	if trueContainer.State.GetExitCode() != 0 {
		t.Errorf("Unexpected exit code %d (expected 0)", trueContainer.State.GetExitCode())
	}
	...
	if falseContainer.State.GetExitCode() != 1 {
		t.Errorf("Unexpected exit code %d (expected 1)", falseContainer.State.GetExitCode())
	}
}
...
func TestUser(t *testing.T) {
	...
	defer runtime.Destroy(container)
	output, err = container.Output()
	if err != nil || container.State.GetExitCode() != 0 {
		t.Fatal(err)
	}
	if !strings.Contains(string(output), "uid=0(root) gid=0(root)") {
		t.Error(string(output))
	...
	if err != nil {
		t.Fatal(err)
	} else if container.State.GetExitCode() != 0 {
		t.Fatalf("Container exit code is invalid: %d\nOutput:\n%s\n", container.State.GetExitCode(), output)
	}
