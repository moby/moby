...
func buildImages(context testContextTemplate, t *testing.T, eng *engine.Engine, useCache bool, repoName string) (map[string]*docker.Image, error) {
	if eng == nil {
		eng = NewTestEngine(t)
	...
	buildfile := docker.NewBuildFile(srv, ioutil.Discard, ioutil.Discard, false, useCache, false, ioutil.Discard, utils.NewStreamFormatter(false), nil, repoName)
...
func TestBuildFailsDockerfileEmpty(t *testing.T) {
	_, err := buildImages(testContextTemplate{``, nil, nil}, t, nil, true, "")
...
func TestBuildTags(t *testing.T) {
	...
	images, err := buildImages(template, t, eng, true, "testing")
	if err != nil {
		t.Fatal(err)
	}
