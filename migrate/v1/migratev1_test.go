...
func (s *DockerSuite) TestMigrateRefs(c *check.C) {
	...
	if !reflect.DeepEqual(expected, ta.refs) {
		c.Fatalf("Invalid migrated tags: expected %q, got %q", expected, ta.refs)
	}
...
func (s *DockerSuite) TestMigrateContainers(c *check.C) {
	// TODO Windows: Figure out why this is failing
	if runtime.GOOS == "windows" {
		c.Skip("Failing on Windows")
	}
	if runtime.GOARCH != "amd64" {
		c.Skip("Test tailored to amd64 architecture")
	}
	...
	if !reflect.DeepEqual(expected, ls.mounts) {
		c.Fatalf("invalid mounts: expected %q, got %q", expected, ls.mounts)
	}

	if actual, expected := ls.count, 0; actual != expected {
		c.Fatalf("invalid active mounts: expected %d, got %d", expected, actual)
	}
	...
	if actual, expected := config.Image, string(imgID); actual != expected {
		c.Fatalf("invalid image pointer in migrated config: expected %q, got %q", expected, actual)
	}

}

func (s *DockerSuite) TestMigrateImages(c *check.C) {
	// TODO Windows: Figure out why this is failing
	if runtime.GOOS == "windows" {
		c.Skip("Failing on Windows")
	}
	...
	if !reflect.DeepEqual(mappings, expected) {
		c.Fatalf("invalid image mappings: expected %q, got %q", expected, mappings)
	}

	if actual, expected := ls.count, 2; actual != expected {
		c.Fatalf("invalid register count: expected %q, got %q", expected, actual)
	}
	ls.count = 0
	...
	if !reflect.DeepEqual(expectedMetadata, receivedMetadata) {
		c.Fatalf("invalid metadata: expected %q, got %q", expectedMetadata, receivedMetadata)
	}

}

func (s *DockerSuite) TestMigrateUnsupported(c *check.C) {
	...
	if err != errUnsupported {
		c.Fatalf("expected unsupported error, got %q", err)
	}
}

func (s *DockerSuite) TestMigrateEmptyDir(c *check.C) {
	tmpdir, err := ioutil.TempDir("", "migrate-empty")
	if err != nil {
		c.Fatal(err)
	}
