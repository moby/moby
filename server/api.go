...
func postAuth(srv *Server, version float64, w http.ResponseWriter, r *http.Request, vars map[string]string) error {
	...
	if status != "" {
		b, err := json.Marshal(&core.APIAuth{Status: status})
		if err != nil {
			return err
		}
		writeJSON(w, b)
...
func postCommit(srv *Server, version float64, w http.ResponseWriter, r *http.Request, vars map[string]string) error {
	...
	b, err := json.Marshal(&core.APIID{id})
	if err != nil {
		return err
	}
	w.WriteHeader(http.StatusCreated)
...
func postImagesInsert(srv *Server, version float64, w http.ResponseWriter, r *http.Request, vars map[string]string) error {
	...
	b, err := json.Marshal(&core.APIID{ID: imgID})
	if err != nil {
		return err
	}
	writeJSON(w, b)
...
func postContainersCreate(srv *Server, version float64, w http.ResponseWriter, r *http.Request, vars map[string]string) error {
	...
	if len(config.Dns) == 0 && len(srv.runtime.Dns) == 0 && utils.CheckLocalDns(resolvConf) {
		out.Warnings = append(out.Warnings, fmt.Sprintf("Docker detected local DNS server on resolv.conf. Using default external servers: %v", core.DefaultDns))
		config.Dns = core.DefaultDns
	}
...
func postContainersWait(srv *Server, version float64, w http.ResponseWriter, r *http.Request, vars map[string]string) error {
	...
	b, err := json.Marshal(&core.APIWait{StatusCode: status})
	if err != nil {
		return err
	}
	writeJSON(w, b)
...
func makeHttpHandler(srv *Server, logging bool, localMethod string, localRoute string, handlerFunc HttpApiFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.Contains(r.Header.Get("User-Agent"), "Docker-Client/") {
			...
			if len(userAgent) == 2 && userAgent[1] != core.VERSION {
				utils.Debugf("Warning: client and server don't have the same version (client: %s, server: %s)", userAgent[1], core.VERSION)
			}
		}
