...
func (srv *Server) DockerVersion() core.APIVersion {
...
func (srv *Server) ImagesSearch(term string) ([]core.APISearch, error) {
	...
	var outs []core.APISearch
	for _, repo := range results.Results {
		var out core.APISearch
...
func (srv *Server) ImageInsert(name, url, path string, out io.Writer, sf *utils.StreamFormatter) (string, error) {
	...
	config, _, _, err := core.ParseRun([]string{img.ID, "echo", "insert", url, path}, srv.runtime.Capabilities)
	if err != nil {
		return "", err
	}
...
func (srv *Server) Images(all bool, filter string) ([]core.APIImages, error) {
	var (
		allImages map[string]*core.Image
		err       error
	)
	if all {
	...
	outs := []core.APIImages{} //produce [] when empty instead of 'null'
	for name, repository := range srv.runtime.Repositories.Repositories {
		if filter != "" && name != filter {
			continue
		}
		for tag, id := range repository {
			var out core.APIImages
...
func (srv *Server) DockerInfo() *core.APIInfo {
	images, _ := srv.runtime.Graph.All()
	var imgcount int
	if images == nil {
		imgcount = 0
...
func (srv *Server) ImageHistory(name string) ([]core.APIHistory, error) {
	...
	outs := []core.APIHistory{} //produce [] when empty instead of 'null'
	err = image.WalkHistory(func(img *core.Image) error {
		var out core.APIHistory
...
func (srv *Server) ContainerTop(name, ps_args string) (*core.APITop, error) {
	if container := srv.runtime.Get(name); container != nil {
...
func (srv *Server) ContainerChanges(name string) ([]core.Change, error) {
	if container := srv.runtime.Get(name); container != nil {
...
func (srv *Server) Containers(all, size bool, n int, since, before string) []core.APIContainers {
	var foundBefore bool
	var displayed int
	retContainers := []core.APIContainers{}
...
func (srv *Server) ContainerCommit(name, repo, tag, author, comment string, config *core.Config) (string, error) {
...
func (srv *Server) pullImage(r *registry.Registry, out io.Writer, imgID, endpoint string, token []string, sf *utils.StreamFormatter) error {
	...
	for _, id := range history {
		if !srv.runtime.Graph.Exists(id) {
			...
			img, err := core.NewImgJSON(imgJSON)
			if err != nil {
...
func (srv *Server) getImageList(localRepo map[string]string) ([]*registry.ImgData, error) {
	...
	for tag, id := range localRepo {
		...
		img.WalkHistory(func(img *core.Image) error {
...
func (srv *Server) pushImage(r *registry.Registry, out io.Writer, remote, imgID, ep string, token []string, sf *utils.StreamFormatter) (checksum string, err error) {
	...
	layerData, err := srv.runtime.Graph.TempLayerArchive(imgID, utils.Uncompressed, sf, out)
	if err != nil {
...
func (srv *Server) ContainerCreate(config *core.Config) (string, error) {
...
func (srv *Server) ContainerDestroy(name string, removeVolume bool) error {
	if container := srv.runtime.Get(name); container != nil {
		...
		if removeVolume {
			...
			usedVolumes := make(map[string]*core.Container)
			for _, container := range srv.runtime.List() {
...
func (srv *Server) deleteImageAndChildren(id string, imgs *[]core.APIRmi) error {
	...
	if len(byParents[id]) == 0 {
		...
		*imgs = append(*imgs, core.APIRmi{Deleted: utils.TruncateID(id)})
...
func (srv *Server) deleteImageParents(img *core.Image, imgs *[]core.APIRmi) error {
	if img.Parent != "" {
...
func (srv *Server) deleteImage(img *core.Image, repoName, tag string) ([]core.APIRmi, error) {
	imgs := []core.APIRmi{}
	...
	if tagDeleted {
		imgs = append(imgs, core.APIRmi{Untagged: img.ShortID()})
...
func (srv *Server) ImageDelete(name string, autoPrune bool) ([]core.APIRmi, error) {
	img, err := srv.runtime.Repositories.LookupImage(name)
	if err != nil {
...
func (srv *Server) ImageGetCached(imgID string, config *core.Config) (*core.Image, error) {
	...
	for elem := range imageMap[imgID] {
		...
		if core.CompareConfig(&img.ContainerConfig, config) {
			return img, nil
		}
	}
	return nil, nil
...
func (srv *Server) ContainerStart(name string, hostConfig *core.HostConfig) error {
	if container := srv.runtime.Get(name); container != nil {
...
func (srv *Server) ContainerInspect(name string) (*core.Container, error) {
	if container := srv.runtime.Get(name); container != nil {
...
func (srv *Server) ImageInspect(name string) (*core.Image, error) {
...
func NewServer(flGraphPath string, autoRestart, enableCors bool, dns core.ListOpts) (*Server, error) {
	if runtime.GOARCH != "amd64" {
		log.Fatalf("The docker runtime currently only supports amd64 (not %s). This will change in the future. Aborting.", runtime.GOARCH)
	}
	runtime, err := core.NewRuntime(flGraphPath, autoRestart, dns)
	if err != nil {
		return nil, err
	}
	srv := &Server{
