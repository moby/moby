...
func (s *DockerSuite) TestContainerApiGetAll(c *check.C) {
	...
	c.Assert(err,checker.IsNil,Commentf("Cannot query container count: %v", err))
	...
	c.Assert(json.Unmarshal(body, &inspectJSON),checker.IsNil,Commentf("unable to unmarshal response body: %v", json.Unmarshal(body, &inspectJSON)))

	c.Assert(len(inspectJSON),checker.Equals,startCount+1,Commentf("Expected %d container(s), %d found (started with: %d)", startCount+1, len(inspectJSON), startCount))

	c.Assert(inspectJSON[0].Names[0], checker.Equals,"/"+name,Commentf("Container Name mismatch. Expected: %q, received: %q\n", "/"+name, inspectJSON[0].Names[0]))
}
...
func (s *DockerSuite) TestContainerApiGetJSONNoFieldsOmitted(c *check.C) {
	...
	for _, f := range fields {
		c.Assert(!strings.Contains(string(body), f),checker.Equals,true,Commentf("Field %s is missing and it shouldn't", f))
	}
}
...
func (s *DockerSuite) TestContainerApiGetExport(c *check.C) {
	...
	c.Assert(found,checker.Equals,true,Commentf("The created test file has not been found in the exported image"))
}
...
func (s *DockerSuite) TestContainerApiGetChanges(c *check.C) {
	...
	c.Assert(json.Unmarshal(body, &changes),checker.IsNil,Commentf("unable to unmarshal response body: %v",json.Unmarshal(body, &changes)))
	...
	c.Assert(success,checker.Equals,true,Commentf("/etc/passwd has been removed but is not present in the diff"))
}
...
func (s *DockerSuite) TestContainerApiStartVolumeBinds(c *check.C) {
	...
	c.Assert(pth,checker.Equals,bindPath,Commentf("expected volume host path to be %s, got %s", bindPath, pth))
}
...
func (s *DockerSuite) TestContainerApiStartDupVolumeBinds(c *check.C) {
	...
	c.Assert(!strings.Contains(string(body), "Duplicate bind"),checker.Equals,true,Commentf("Expected failure due to duplicate bind mounts to same path, instead got: %q with error: %v", string(body), err))
}
...
func (s *DockerSuite) TestContainerApiStartVolumesFrom(c *check.C) {
	...
	c.Assert(pth,checker.Equals,pth2,Commentf("expected volume host path to be %s, got %s", pth, pth2))
}
...
func (s *DockerSuite) TestGetContainerStatsRmRunning(c *check.C) {
	...
	c.Assert(err, check.Not(checker.IsNil), check.Commentf("rm should have failed but didn't"))
	_, err = buf.ReadTimeout(b, 2*time.Second)
	c.Assert(err, checker.IsNil)
	dockerCmd(c, "rm", "-f", id)

	_, err = buf.ReadTimeout(b, 2*time.Second)
	c.Assert(err, check.Not(checker.IsNil))
}
...
func (s *DockerSuite) TestGetContainerStatsNoStream(c *check.C) {
	...
	case sr := <-bc:
		...
		s := string(sr.body)
		// count occurrences of "read" of types.Stats
		c.Assert(strings.Count(s, "read"), checker.Equals, 1, Commentf("Expected only one stat streamed, got %d",strings.Count(s, "read")))
	}
}
...
func (s *DockerSuite) TestPostContainerBindNormalVolume(c *check.C) {
	...
	c.Assert(fooDir2,checker.Equals,fooDir,Commentf("expected volume path to be %s, got: %s", fooDir, fooDir2))
}
...
func (s *DockerSuite) TestContainerApiPause(c *check.C) {
	...
	c.Assert(err, checker.IsNil,Commentf("error thrown while checking if containers were paused: %v", err))
	...
	c.Assert(pausedContainers, checker.IsNil,Commentf("There should be no paused container."))	
}
...
func (s *DockerSuite) TestContainerApiTop(c *check.C) {
	...
	c.Assert(len(top.Titles),checker.Equals,11,Commentf("expected 11 titles, found %d: %v", len(top.Titles), top.Titles))

	if top.Titles[0] != "USER" || top.Titles[10] != "COMMAND" {
		c.Fatalf("expected `USER` at `Titles[0]` and `COMMAND` at Titles[10]: %v", top.Titles)
	}
	c.Assert(len(top.Processes),checker.Equals,2,Commentf("expected 2 processes, found %d: %v", len(top.Processes), top.Processes))
	c.Assert(top.Processes[0][10],checker.Equals,"/bin/sh -c top",Commentf("expected `/bin/sh -c top`, found: %s", top.Processes[0][10]))
	c.Assert(top.Processes[1][10],checker.Equals,"top",Commentf("expected `top`, found: %s", top.Processes[1][10]))
...
func (s *DockerSuite) TestContainerApiCommit(c *check.C) {
	...
	c.Assert(err,checker.IsNil)
	c.Assert(cmd,checker.Equals,"{[/bin/sh -c touch /test]}",Commentf("got wrong Cmd from commit: %q", cmd))
...
func (s *DockerSuite) TestContainerApiBadPort(c *check.C) {
	...
	c.Assert(strings.TrimSpace(string(b)),checker.Equals,`Invalid port specification: "aa80"`,Commentf("Incorrect error msg: %s", string(b)))
}
...
func (s *DockerSuite) TestContainerApiCreate(c *check.C) {
	...
	c.Assert(strings.TrimSpace(out),checker.Equals,"/test",Commentf("expected output `/test`, got %q", out))
}
...
func (s *DockerSuite) TestContainerApiCreateEmptyConfig(c *check.C) {
	...
	c.Assert(string(b),checker.Equals,expected,Commentf("Expected to get %q, got %q", expected,string(b)))
}
...
func (s *DockerSuite) TestContainerApiCreateWithHostName(c *check.C) {
	...
	c.Assert(containerJSON.Config.Hostname,checker.Equals,hostName,Commentf("Mismatched Hostname, Expected %s, Actual: %s ", hostName, containerJSON.Config.Hostname))
}
...
func (s *DockerSuite) TestContainerApiCreateWithDomainName(c *check.C) {
	...
	c.Assert(containerJSON.Config.Domainname,checker.Equals,domainName,Commentf("Mismatched Domainname, Expected %s, Actual: %s ", domainName, containerJSON.Config.Domainname))
}
...
func UtilCreateNetworkMode(c *check.C, networkMode string) {
	...
	c.Assert(containerJSON.HostConfig.NetworkMode,checker.Equals,runconfig.NetworkMode(networkMode),Commentf("Mismatched NetworkMode, Expected %s, Actual: %s ", networkMode, containerJSON.HostConfig.NetworkMode))
}
...
func (s *DockerSuite) TestContainerApiCreateWithCpuSharesCpuset(c *check.C) {
	...
	c.Assert(outCpuset, checker.Equals, "0,1")
}
...
func (s *DockerSuite) TestContainerApiInvalidPortSyntax(c *check.C) {
	...
	b, err := readBody(body)
	c.Assert(err, checker.IsNil)
	c.Assert(strings.Contains(string(b[:]), "Invalid port"), checker.Equals, true)
}
...
func (s *DockerSuite) TestContainerApiPostCreateNull(c *check.C) {
	...
	c.Assert(err, checker.IsNil)
	c.Assert(out, checker.Equals,"",Commentf("expected empty string, got %q", out))
...
func (s *DockerSuite) TestCreateWithTooLowMemoryLimit(c *check.C) {
	...
	c.Assert(res.StatusCode, checker.Equals, http.StatusInternalServerError)
	c.Assert(strings.Contains(string(b), "Minimum memory limit allowed is 4MB"), checker.Equals, true)
}
...
func (s *DockerSuite) TestContainerApiRename(c *check.C) {
	...
	c.Assert(name,checker.Equals,"/"+newName,Commentf("Failed to rename container, expected %v, got %v. Container rename API failed", newName, name))
}
...
func (s *DockerSuite) TestContainerApiKill(c *check.C) {
	...
	c.Assert(err, checker.IsNil)
	c.Assert(state,checker.Equals,"false",Commentf("got wrong State from container %s: %q", name, state))
}
...
func (s *DockerSuite) TestContainerApiWait(c *check.C) {
	...
	c.Assert(json.Unmarshal(body, &waitres), checker.IsNil,Commentf("unable to unmarshal response body: %v",json.Unmarshal(body, &waitres)))

	c.Assert(waitres.StatusCode,checker.Equals,0,Commentf("Expected wait response StatusCode to be 0, got %d", waitres.StatusCode))
}
...
func (s *DockerSuite) TestContainerApiDeleteRemoveLinks(c *check.C) {
	...
	c.Assert(links,checker.Equals,"[\"/tlink1:/tlink2/tlink1\"]",Commentf("expected to have links between containers"))
	...
	c.Assert(linksPostRm,checker.Equals,"null",Commentf("call to api deleteContainer links should have removed the specified links"))
...
func (s *DockerSuite) TestContainerApiDeleteRemoveVolume(c *check.C) {
	...
	_, err := os.Stat(source)
	Assert(os.IsNotExist(err),checker.Equals,false,Commentf("expected to get ErrNotExist error, got %v", err))
...
func (s *DockerSuite) TestContainersApiChunkedEncoding(c *check.C) {
	...
	c.Assert(err, checker.IsNil,Commentf("error starting container with chunked encoding: %v", err))
	resp.Body.Close()
	c.Assert(resp.StatusCode,checker.Equals,204,Commentf("expected status code 204, got %d", resp.StatusCode))
	...
	c.Assert(binds,checker.hasLen,1,"got unexpected binds: %v", binds)

	expected := "/tmp:/foo"
	c.Assert(binds[0],checker.Equals,expected,Commentf("got incorrect bind spec, wanted %s, got: %s", expected, binds[0]))
}
...
func (s *DockerSuite) TestContainersApiGetContainersJSONEmpty(c *check.C) {
	...
	c.Assert(err, checker.IsNil)
	c.Assert(status, checker.Equals, http.StatusOK)
	c.Assert(string(body),checker.Equals,"[]\n","Expected empty response to be `[]`, got %q", string(body))
}
...
func (s *DockerSuite) TestPostContainersCreateWithWrongCpusetValues(c *check.C) {
	...
	c.Assert(string(body), checker.Equals, expected, check.Commentf("Expected output to contain %q, got %q", expected, string(body)))
