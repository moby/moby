...
func (s *DockerHubPullSuite) TestPullFromCentralRegistry(c *check.C) {
	...
	c.Assert(out, checker.Contains, "Downloaded newer image for hello-world@sha256:")
	c.Assert(out, checker.Contains, "Tagging hello-world@sha256:")
	c.Assert(out, checker.Contains, "as hello-world:latest")
	...
	c.Assert(splitImg, checker.HasLen, 3) // the tagged version always appears to show above the <none> entry
...
func (s *DockerHubPullSuite) TestPullNonExistingImage(c *check.C) {
	...
	for record := range recordChan {
		if len(record.option) == 0 {
			...
			c.Assert(record.out, checker.Contains, "you are not authorized to perform this operation: server returned 401.", check.Commentf("expected image not found error messages"))
		} else {
			// pull -a on a nonexistent registry should fall back as well
			c.Assert(record.err, checker.NotNil, check.Commentf("expected non-zero exit status when pulling non-existing image: %s", record.out))
			c.Assert(record.out, checker.Contains, "you are not authorized to perform this operation: server returned 401.", check.Commentf("expected image not found error messages"))
...
func (s *DockerHubPullSuite) TestPullFromCentralRegistryImplicitRefParts(c *check.C) {
	...
	for _, i := range []string{
		...
		c.Assert(out, checker.Contains, "Image is up to date for hello-world@sha256")
	}
...
func (s *DockerHubPullSuite) TestPullScratchNotAllowed(c *check.C) {
	...
	c.Assert(out, checker.Contains, "remote trust data does not exist for docker.io/library/scratch")
...
func (s *DockerHubPullSuite) TestPullAllTagsFromCentralRegistry(c *check.C) {
	...
	c.Assert(splitOutImageCmd, checker.HasLen, 3)
