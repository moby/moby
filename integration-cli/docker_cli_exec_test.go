...
func TestExecCleanupOrphanedTasks(t *testing.T) {
	defer deleteAllContainers()
	name := "test_exec_orphan"
	defer deleteImages(name)

	_, err := buildImage(name, `
	FROM busybox
	# close all stdin/stdout/stderr so exec cmd can returns without
	# waiting for file descriptors to be released
	RUN echo -e 'exec 0>&-\nexec 1>&-\nexec 2>&-\n(sleep 5 &)\n' > /bin/testexec
	RUN chmod +x /bin/testexec
	CMD sleep 100
	`, true)
	if err != nil {
		t.Fatal(err)
	...
	out, err := exec.Command(dockerBinary, "run", "-d", "--name", "testing", name).CombinedOutput()
	if err != nil {
		t.Fatal(string(out), err)
	}

	err = exec.Command(dockerBinary, "exec", "testing", "sh", "-c", "/bin/testexec").Run()
	if err != nil {
		t.Fatal(string(out), err)
	}

	out, err = exec.Command(dockerBinary, "exec", "testing", "ps").CombinedOutput()
	if err != nil {
		t.Fatal(string(out), err)
	}
	if strings.Contains(string(out), "sleep 5") {
		t.Fatalf("failed to clean orphaned tasks from previous exec job. ps %s", string(out))
	}

	logDone("exec - cleanup orphaned tasks generated by exec job")
}
