...
func (s *DockerDaemonSuite) TestRunWithRuntimeFromCommandLine(c *check.C) {
	s.d.StartWithBusybox(c, "--add-runtime", "oci=moby-runc", "--add-runtime", "vm=/usr/local/bin/vm-manager")
	...
	// Check that we can select a default runtime
	s.d.Stop(c)
	s.d.StartWithBusybox(c, "--default-runtime=vm", "--add-runtime", "oci=moby-runc", "--add-runtime", "vm=/usr/local/bin/vm-manager")
