...
func (s *DockerSuite) TestPause(c *check.C) {
	...
	c.Assert(len(pausedContainers), check.Equals, 1)

	dockerCmd(c, "unpause", name)

	out, _ := dockerCmd(c, "events", "--since=0", fmt.Sprintf("--until=%d", daemonTime(c).Unix()))
	events := strings.Split(out, "\n")
	c.Assert(len(events) > 1, check.Equals, true)

	pauseEvent := strings.Fields(events[len(events)-3])
	unpauseEvent := strings.Fields(events[len(events)-2])

	c.Assert(pauseEvent[len(pauseEvent)-1], check.Equals, "pause")
	c.Assert(unpauseEvent[len(unpauseEvent)-1], check.Equals, "unpause")
...
func (s *DockerSuite) TestPauseMultipleContainers(c *check.C) {
	...
	pausedContainers, err := getSliceOfPausedContainers()
	c.Assert(err, check.IsNil)
	c.Assert(len(pausedContainers), check.Equals, len(containers))

	dockerCmd(c, append([]string{"unpause"}, containers...)...)

	out, _ := dockerCmd(c, "events", "--since=0", fmt.Sprintf("--until=%d", daemonTime(c).Unix()))
	events := strings.Split(out, "\n")
	c.Assert(len(events) > len(containers)*3-2, check.Equals, true)
