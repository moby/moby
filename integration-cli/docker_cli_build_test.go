...
func (s *DockerSuite) TestBuildHandleEscapes(c *check.C) {
  ...
  `, true)
	...
	if _, ok := result["bar"]; !ok {
		c.Fatalf("Could not find volume bar set from env foo in volumes table, got %s", result)
	}
  ...
  `, true)
	...
	if _, ok := result["${FOO}"]; !ok {
		c.Fatalf("Could not find volume ${FOO} set from env foo in volumes table, got %s", result)
	}
  ...
  `, true)
	...
	if _, ok := result[`\\\${FOO}`]; !ok {
		c.Fatalf("ould not find volume \\\${FOO} set from env foo in volumes table, got %s", result)
	}
