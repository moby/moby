...
func makeTestContainerWithCommand(t *testing.T, addContent bool, volumes []string, workDir, command string) (containerID string) {
	if addContent {
		mkContentCmd := defaultMkContentCommand()
		if command == "" {
			command = mkContentCmd
		} else {
			command = fmt.Sprintf("%s && %s", defaultMkContentCommand(), command)
		}
	}
	...
	args = append(args, "busybox", "/bin/sh", "-c", command)
...
func makeTestContainer(t *testing.T, addContent bool, volumes []string, workDir string) (containerID string) {
	return makeTestContainerWithCommand(t, addContent, volumes, workDir, "")
}

func makeTestContainerCatFile(t *testing.T, addContent bool, volumes []string, workDir, path string) (containerID string) {
	command := fmt.Sprintf("if [ -f %s ]; then cat %s; fi", path, path)

	return makeTestContainerWithCommand(t, addContent, volumes, workDir, command)
}
