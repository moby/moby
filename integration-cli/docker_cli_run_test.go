...
func (s *DockerSuite) TestRunWithSwappinessInvalid(c *check.C) {
	...
	out, _, err = dockerCmdWithError(c, "run", "--memory-swappiness", "-1", "busybox", "true")
	if err == nil {
		c.Fatalf("failed. test was able to set invalid value, output: %q", out)
	}
}
...
func (s *DockerSuite) TestRunEchoStdoutWithCPUAndMemoryLimit(c *check.C) {
	out, _ := dockerCmd(c, "run", "-c", "1000", "-m", "16m", "busybox", "echo", "test")
	if out != "test\n" {
...
func (s *DockerSuite) TestRunLookupGoogleDns(c *check.C) {
	...
	out, _ := dockerCmd(c, "run", "busybox", "nslookup", "google.com")
}
...
func (s *DockerSuite) TestRunExitCodeOne(c *check.C) {
	out, exitCode, err := dockerCmdWithError(c, "run", "busybox", "false")
...
func (s *DockerSuite) TestRunWithoutNetworking(c *check.C) {
	out, exitCode := dockerCmd(c, "run", "--net=none", "busybox", "ping", "-c", "1", "8.8.8.8")
	if exitCode != 1 {
		c.Errorf("--net=none should've disabled the network; the container shouldn't have been able to ping 8.8.8.8")
	}

	out, exitCode = dockerCmd(c, "run", "-n=false", "busybox", "ping", "-c", "1", "8.8.8.8")
	if exitCode != 1 {
...
func (s *DockerSuite) TestRunSysNotWritableInNonPrivilegedContainers(c *check.C) {
	if out, code, err := dockerCmdWithError(c, "run", "busybox", "touch", "/sys/kernel/profiling"); err == nil || code == 0 {
...
func (s *DockerSuite) TestRunSysWritableInPrivilegedContainers(c *check.C) {
	if out, code, err := dockerCmdWithError(c, "run", "--privileged", "busybox", "touch", "/sys/kernel/profiling"); err != nil || code != 0 {
...
func (s *DockerSuite) TestRunUnprivilegedWithChroot(c *check.C) {
	out, _ := dockerCmd(c, "run", "busybox", "chroot", "/", "true")
}
...
func (s *DockerSuite) TestRunDnsOptions(c *check.C) {
	...
	out, _ = dockerCmd(c, "run", "--dns=127.0.0.1", "--dns-search=.", "busybox", "cat", "/etc/resolv.conf")
...
func (s *DockerSuite) TestRunUnknownCommand(c *check.C) {
	testRequires(c, NativeExecDriver)
	out, _ := dockerCmd(c, "create", "busybox", "/bin/nada")

	id = strings.TrimSpace(out)
	dockerCmd(c, "start", id)

	rc, err := inspectField(id, "State.ExitCode")
	c.Assert(err, check.IsNil)
	if rc == "0" {
...
func (s *DockerSuite) TestRunTLSverify(c *check.C) {
	if out, code, err := dockerCmdWithError(c, "ps"); err != nil || ec != 0 {
...
func (s *DockerSuite) TestMountIntoProc(c *check.C) {
	testRequires(c, NativeExecDriver)
	out, code, err := dockerCmdWithError(c, "run", "-v", "/proc//sys", "busybox", "true")
	if err == nil || code == 0 {
