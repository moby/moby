...
func (s *DockerRegistrySuite) TestPullFailsWithAlteredManifest(c *check.C) {
	...
	defer s.reg.tempMoveBlobData(c, manifestDigest)()
...
func (s *DockerRegistrySuite) TestPullFailsWithAlteredLayer(c *check.C) {
	...
	// Move the existing data file aside, so that we can replace it with a
	// malicious blob of data. NOTE: we defer the returned undo func.
	defer s.reg.tempMoveBlobData(c, targetLayerDigest)()
	...
	expectedErrorMsg := fmt.Sprintf("image verification failed for digest %s", targetLayerDigest)
	if !strings.Contains(out, expectedErrorMsg) {
		c.Fatalf("expected error message %q in output: %s", expectedErrorMsg, out)
	}
}
