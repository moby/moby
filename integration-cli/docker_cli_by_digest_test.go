...
func (s *DockerRegistrySuite) TestBuildByDigest(t *testing.T) {
	skip.If(t, testEnv.UsingSnapshotter(), "Config.Image is not created with containerd, buildkit doesn't set it either")
	imgDigest, err := setupImage(t)
	assert.NilError(t, err, "error setting up image")
	...
	res := inspectField(t, name, "Config.Image")
	// make sure they match
	assert.Equal(t, res, imageID)
}
