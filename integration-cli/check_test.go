...
func (s *DockerSchema1RegistrySuite) OnTimeout(t *testing.T) {
	s.d.DumpStackAndQuit()
}

func (s *DockerSchema1RegistrySuite) SetUpTest(ctx context.Context, t *testing.T) {
	testRequires(t, DaemonIsLinux, RegistryHosting, NotArm64, testEnv.IsLocalDaemon)
	s.reg = registry.NewV2(t, registry.Schema1)
	s.reg.WaitReady(t)
	s.d = daemon.New(t, dockerBinary, dockerdBinary, testdaemon.WithEnvironment(testEnv.Execution))
}

func (s *DockerSchema1RegistrySuite) TearDownTest(ctx context.Context, t *testing.T) {
	if s.reg != nil {
		s.reg.Close()
	}
	if s.d != nil {
