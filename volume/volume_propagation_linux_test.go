...
func TestParseMountSpecPropagation(t *testing.T) {
	...
	valid = []string{
		...
		"/hostPath:/containerPath:ro:foo=bar",
		"/hostPath:/containerPath:ro:foo=bar,baz=bat",
		"/hostPath:/containerPath:ro:ip=bat:1234",
		"/hostPath:/containerPath:ro:ip=http://localhost:1234,addr=0.0.0.0:1234",
		"/hostPath:/containerPath:ro:dfs-addr=1.2.3.4:9999,keyfile=/etc/conf/dfs/creds.keyfile",
	}
	invalid = map[string]string{
		...
		"/path:/path:ro,foo":                `invalid mode: ro,foo`,
		"/hostPath:/containerPath::foo=bar": "Invalid volume specification",
	}
