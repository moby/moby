...
func TestServicePruneDryRun_LabelFilter(t *testing.T) {
	t.Parallel()
	...
	v, err := service.Get(ctx, "test")
	assert.NilError(t, err)
	assert.Assert(t, is.Equal(v.Driver, volume.DefaultDriverName))
	v, err = service.Get(ctx, "test2")
	assert.NilError(t, err)
	assert.Assert(t, is.Equal(v.Driver, "other"))

	// Remove 'test' volume so that we can recreate it with a new label
	err = service.Remove(ctx, "test")
	assert.NilError(t, err)
	_, err = service.Create(ctx, "test", volume.DefaultDriverName, opts.WithCreateLabels(map[string]string{"banana": ""}))
	assert.NilError(t, err)
	...
	assert.Assert(t, is.Len(pr.VolumesDeleted, 1))
	assert.Assert(t, is.Equal(pr.VolumesDeleted[0], "test3"))
	v, err = service.Get(ctx, "test3")
	assert.NilError(t, err)
}

func TestServicePruneDryRun_Reference(t *testing.T) {
	t.Parallel()
