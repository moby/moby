...
do_install() {
	...
	case "$lsb_dist" in
		ubuntu|debian|raspbian)
			...
			(
			set -x
			curl -sSL "${download_url}/${lsb_dist}/gpg" | $sh_c 'apt-key add -'
			$sh_c "mkdir -p /etc/apt/sources.list.d"
			$sh_c "echo deb \[arch=$(dpkg --print-architecture)\] ${download_url}/${lsb_dist} ${dist_version} ${channel} > /etc/apt/sources.list.d/docker.list"
			$sh_c 'sleep 3; apt-get update; apt-get install -y -q docker-ce'
			)
			echo_docker_as_nonroot
			exit 0
			;;
		...
		fedora|centos|redhat|oraclelinux|photon)
			...
			curl -sSL "${download_url}/${lsb_dist}/docker-ce.repo" > /etc/yum.repos.d/docker-ce-${channel}.repo

			if [ "$lsb_dist" = "fedora" ] && [ "$dist_version" -ge "24" ]; then
				(
					set -x
					$sh_c 'sleep 3; dnf -y -q install docker-ce'
				)
			elif [ "$lsb_dist" = "photon" ]; then
				(
					set -x
					$sh_c 'sleep 3; tdnf -y install docker-ce'
				)
			else
				(
					set -x
					$sh_c 'sleep 3; yum -y -q install docker-ce'
				)
			fi
			echo_docker_as_nonroot
			exit 0
