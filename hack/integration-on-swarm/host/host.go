...
const (
	defaultStackName       = "integration-on-swarm"
	defaultVolumeName      = "integration-on-swarm"
	defaultMasterImageName = "integration-master"
	defaultWorkerImageName = "integration-worker"
)
...
func ensureImages(cli *client.Client, images []string) error {
	for _, image := range images {
		...
		if err != nil {
			return fmt.Errorf("could not find image %s, please run `make build-integration-on-swarm`: %v",
				image, err)
		}
	}
	return nil
